...
function admin_payload(): array {
    ...
    $payload=['ok'=>true,
        ...
        ,'service_addons'=>bluegate_service_addon_settings()
    ];
...
if ($action === 'admin_service_plan_addons' || $action === 'admin_service_addons_global') { require_admin($user);try{$cfg=bluegate_save_service_addon_settings($input);log_admin_action((int)$user['telegram_id'],'service_addons_global','system',1);api_out(['ok'=>true,'service_addons'=>$cfg,'catalog'=>catalog_public_payload()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'تنظیم مشترک تمدید و افزونه ناموفق بود.'],400);} }
