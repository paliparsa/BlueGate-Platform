...
if ($action === 'renew_order') { $oid=(int)($input['order_id']??0);$old=order_by_id($oid);if(!$old||(int)$old['user_id']!==(int)$user['id'])api_out(['ok'=>false,'message'=>'سفارش قبلی پیدا نشد.'],404);try{$order=create_storefront_order((int)$user['id'],(int)$old['product_id'],!empty($old['variant_id'])?(int)$old['variant_id']:null,null);notify_admins("🔄 تمدید سرویس از Mini App\nسفارش جدید: <code>#{$order['id']}</code>\nسفارش قبلی: <code>#{$oid}</code>\nمحصول: <b>".h(order_catalog_display_name($order))."</b>");api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['order'=>order_public_payload($order)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'این پلن در حال حاضر قابل تمدید نیست.'],400);} }
if ($action === 'my_orders') { api_out(['ok'=>true, 'orders'=>array_map('order_public_payload', user_orders((int)$user['id'], 50)), 'services'=>user_active_services((int)$user['id'],12), 'notifications'=>user_notifications((int)$user['id'],30), 'notification_unread'=>user_notification_unread_count((int)$user['id'])]); }
...
if ($action === 'cancel_order') { $orderId=(int)($input['order_id']??0); $order=order_by_id($orderId); if(!$order || (int)$order['user_id'] !== (int)$user['id'] || normalize_order_status($order['status']) !== 'pending_payment') api_out(['ok'=>false,'error'=>'ORDER_LOCKED','message'=>'امکان لغو این سفارش نیست.'],400); cancel_order($orderId,'لغو از وب‌سایت/Mini App'); api_out(dashboard_payload(get_user_by_id((int)$user['id']))); }
...
/* v2.7 Credit Center actions */
if ($action === 'create_credit_topup') { api_rate_limit('credit_topup_create',(string)$user['id'],10,600,600); try{$t=create_credit_topup((int)$user['id'],(int)($input['amount']??0));api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['topup'=>credit_topup_public($t)]);}catch(Throwable $e){$code=api_exception_code($e);$msg=$code==='TOPUP_BELOW_MIN'?'مبلغ شارژ از حداقل مجاز کمتر است.':($code==='TOPUP_ABOVE_MAX'?'مبلغ شارژ از حداکثر مجاز بیشتر است.':'امکان ساخت درخواست شارژ نیست.');api_out(['ok'=>false,'error'=>$code,'message'=>$msg],400);} }
if ($action === 'credit_topup_set_method') { api_rate_limit('credit_topup_method',(string)$user['id'],20,600,600); try{$t=set_credit_topup_method((int)($input['topup_id']??0),(int)$user['id'],(string)($input['method']??''),is_array($input['details']??null)?$input['details']:[]);api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['topup'=>credit_topup_public($t)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'این روش پرداخت برای شارژ حساب قابل استفاده نیست.'],400);} }
if ($action === 'credit_topup_start_stars') { api_rate_limit('credit_topup_stars',(string)$user['id'],6,600,600); try{$t=set_credit_topup_method((int)($input['topup_id']??0),(int)$user['id'],'stars',[]);$res=send_stars_invoice_for_topup($t);if(empty($res['ok']))throw new RuntimeException('STARS_INVOICE_FAILED');api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['topup'=>credit_topup_public(credit_topup_by_id((int)$t['id'])),'stars_invoice_sent'=>true]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ارسال فاکتور Stars برای شارژ انجام نشد.'],400);} }
if ($action === 'credit_topup_submit_crypto_hash') { api_rate_limit('credit_topup_crypto',(string)$user['id'],12,900,900); try{$t=submit_credit_topup_crypto_hash((int)($input['topup_id']??0),(int)$user['id'],(string)($input['tx_hash']??''));notify_admins('🪙 شارژ اعتبار در انتظار بررسی\\nTop-up: <code>#'.$t['id'].'</code>\\nمبلغ: <b>'.money((int)$t['amount']).'</b>\\nTXID: <code>'.h((string)$t['tx_hash']).'</code>');api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['topup'=>credit_topup_public($t)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ثبت TXID انجام نشد.'],400);} }
