...
function api_cookie_token(): string { return trim((string)($_COOKIE['nx_session']??'')); }
function api_set_session_cookie(string $token, bool $remember=true): void {$secure=(!empty($_SERVER['HTTPS'])&&strtolower((string)$_SERVER['HTTPS'])!=='off')||((string)($_SERVER['HTTP_X_FORWARDED_PROTO']??'')==='https');$origin=trim((string)app_config('WEB_ALLOWED_ORIGIN',''));$apiHost=strtolower((string)($_SERVER['HTTP_HOST']??''));$originHost=strtolower((string)(parse_url($origin,PHP_URL_HOST)?:''));$sameSite=($originHost&&$apiHost&&!str_contains($apiHost,$originHost)&&!str_contains($originHost,preg_replace('/:\d+$/','',$apiHost)))?'None':'Lax';$expires=$remember?time()+max(1,setting_int('auth_token_ttl_hours',168))*3600:0;setcookie('nx_session',$token,['expires'=>$expires,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>$sameSite]);}
function api_clear_session_cookie(): void { setcookie('nx_session','',['expires'=>time()-3600,'path'=>'/','secure'=>!empty($_SERVER['HTTPS']),'httponly'=>true,'samesite'=>'Lax']); }
...
function admin_payload(): array {
    ...
    $payload=['ok'=>true,
        ...
        'notification_campaigns'=>admin_notification_campaigns(60)
        ,'infrastructure'=>bluegate_infrastructure_summary()
        ,'service_plan_provider_maps'=>bluegate_plan_provider_maps()
        ,'provisioning_jobs'=>bluegate_provisioning_jobs(60)
        ,'monitoring'=>bluegate_monitoring_dashboard()
        ,'routing'=>bluegate_routing_dashboard()
        ,'advanced'=>bluegate_advanced_dashboard()
    ];
...
if ($action === 'mini_enhancements') {
    ...
    api_out([
        'ok'=>true,
        'services'=>safe_mini_optional(fn()=>bluegate_user_services((int)$user['id'],20),active_services_from_order_payloads($op,12)),
...
if ($action === 'my_services') { api_out(['ok'=>true,'services'=>bluegate_user_services((int)$user['id'],50)]); }
if ($action === 'service_details') { try{$sid=(int)($input['service_id']??0);$s=bluegate_service_record($sid,(int)$user['id']);if(!$s)api_out(['ok'=>false,'error'=>'SERVICE_NOT_FOUND','message'=>'سرویس پیدا نشد.'],404);api_out(['ok'=>true,'service'=>bluegate_service_public($s,true)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'دریافت جزئیات سرویس ناموفق بود.'],400);} }
if ($action === 'service_sync') { try{$sid=(int)($input['service_id']??0);$s=bluegate_service_record($sid,(int)$user['id']);if(!$s)api_out(['ok'=>false,'error'=>'SERVICE_NOT_FOUND','message'=>'سرویس پیدا نشد.'],404);api_out(['ok'=>true,'service'=>bluegate_sync_service($sid,true)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'همگام‌سازی سرویس انجام نشد.'],400);} }
if ($action === 'service_renew') { try{$sid=(int)($input['service_id']??0);$s=bluegate_service_record($sid,(int)$user['id']);if(!$s)throw new RuntimeException('SERVICE_NOT_FOUND');$old=$s['order_id']?order_by_id((int)$s['order_id']):null;if(!$old)throw new RuntimeException('SERVICE_ORDER_NOT_FOUND');$order=create_renewal_order_from_order((int)$user['id'],(int)$old['id']);db()->prepare('UPDATE orders SET user_service_id=?,service_action="renew",service_action_json=? WHERE id=?')->execute([$sid,json_encode(['service_id'=>$sid],JSON_UNESCAPED_SLASHES),(int)$order['id']]);api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['order'=>order_public_payload(order_by_id((int)$order['id']))]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ساخت سفارش تمدید ممکن نشد.'],400);} }
if ($action === 'service_add_traffic') { try{$order=bluegate_service_action_order((int)$user['id'],(int)($input['service_id']??0),'add_traffic',(int)($input['gb']??0));api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['order'=>order_public_payload($order)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'خرید حجم اضافه ممکن نیست.'],400);} }
if ($action === 'service_add_days') { try{$order=bluegate_service_action_order((int)$user['id'],(int)($input['service_id']??0),'add_days',(int)($input['days']??0));api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['order'=>order_public_payload($order)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'خرید زمان اضافه ممکن نیست.'],400);} }
...
if ($action === 'welcome_seen') { $version=max(0,min(100,(int)($input['version']??1)));db()->prepare('UPDATE users SET welcome_version_seen=GREATEST(COALESCE(welcome_version_seen,0),?) WHERE id=?')->execute([$version,(int)$user['id']]);$fresh=get_user_by_id((int)$user['id']);api_out(['ok'=>true,'welcome_version_seen'=>(int)($fresh['welcome_version_seen']??$version),'user'=>user_payload($fresh)]); }
...
if ($action === 'my_orders') { $op=array_map('order_public_payload', user_orders((int)$user['id'],50)); api_out(['ok'=>true, 'orders'=>$op, 'services'=>safe_mini_optional(fn()=>bluegate_user_services((int)$user['id'],20),active_services_from_order_payloads($op,12)), 'notifications'=>safe_mini_optional(fn()=>user_notifications((int)$user['id'],30),[]), 'notification_unread'=>(int)safe_mini_optional(fn()=>user_notification_unread_count((int)$user['id']),0)]); }
...
if ($action === 'cancel_order') {
    ...
    if(!in_array($status,['pending_payment','receipt_submitted','reviewing'],true))api_out(['ok'=>false,'error'=>'ORDER_NOT_CANCELABLE','message'=>'این سفارش دیگر قابل لغو نیست.'],400);
    refund_wallet_for_order($oid,'بازگشت کیف پول سفارش لغوشده توسط کاربر');
    $updated=cancel_order($oid,'لغو توسط کاربر');
    if(!$updated)api_out(['ok'=>false,'error'=>'ORDER_CANCEL_FAILED','message'=>'لغو سفارش انجام نشد.'],400);
    api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['order'=>order_public_payload($updated)]);
}
if ($action === 'hide_order') {
    $oid=(int)($input['order_id']??0);
    if(!hide_user_order($oid,(int)$user['id']))api_out(['ok'=>false,'error'=>'ORDER_NOT_HIDEABLE','message'=>'فقط سفارش‌های لغو، رد یا مرجوع‌شده قابل حذف از لیست هستند.'],400);
    api_out(dashboard_payload(get_user_by_id((int)$user['id'])));
}
if ($action === 'clear_canceled_orders') {
    $count=hide_user_cleanup_orders((int)$user['id']);
    api_out(dashboard_payload(get_user_by_id((int)$user['id']))+['hidden_count'=>$count]);
}
...
if ($action === 'submit_receipt') {
    ...
    try{
        ...
        foreach(app_config('ADMIN_IDS',[]) as $aid){
            if($fileId){$path=__DIR__.'/'.$fileId;if(file_exists($path)){try{tg('sendPhoto',['chat_id'=>$aid,'photo'=>new CURLFile($path),'caption'=>$adminText,'parse_mode'=>'HTML','reply_markup'=>admin_order_keyboard((int)$order['id'])]);}catch(Throwable $e){send_msg($aid,$adminText,admin_order_keyboard((int)$order['id']));}}else send_msg($aid,$adminText,admin_order_keyboard((int)$order['id']));}else send_msg($aid,$adminText,admin_order_keyboard((int)$order['id']));
        }
...
if ($action === 'admin_infrastructure_summary') {
    require_admin($user);
    api_out(['ok'=>true,'infrastructure'=>bluegate_infrastructure_summary()]);
}
if ($action === 'admin_provider_save') {
    require_admin($user);
    try {
        $input['is_active']=array_key_exists('is_active',$input)?bool_input($input['is_active']):1;
        $id=bluegate_save_provider($input);
        log_admin_action((int)$user['telegram_id'],'provider_save','service_provider',$id,(string)($input['name']??''));
        api_out(admin_payload()+['provider_id'=>$id,'message'=>'Provider ذخیره شد.']);
    } catch(Throwable $e) { api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ذخیره Provider ناموفق بود: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_test') {
    require_admin($user);
    try { $r=bluegate_test_provider((int)($input['provider_id']??0)); api_out(['ok'=>true,'result'=>$r,'infrastructure'=>bluegate_infrastructure_summary()]); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'تست Provider انجام نشد: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_toggle') {
    require_admin($user);$id=(int)($input['provider_id']??0);$p=bluegate_provider($id,true);if(!$p)api_out(['ok'=>false,'message'=>'Provider پیدا نشد.'],404);
    db()->prepare('UPDATE service_providers SET is_active=1-is_active WHERE id=?')->execute([$id]);
    log_admin_action((int)$user['telegram_id'],'provider_toggle','service_provider',$id);
    api_out(admin_payload());
}

if ($action === 'admin_provider_inbounds') {
    require_admin($user); $id=(int)($input['provider_id']??0);
    try { api_out(['ok'=>true,'provider'=>bluegate_provider($id,false),'inbounds'=>bluegate_provider_inbounds($id)]); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'دریافت Inboundها ناموفق بود: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_client_get') {
    require_admin($user); $id=(int)($input['provider_id']??0); $external=trim((string)($input['external_id']??$input['email']??''));
    if($external==='') api_out(['ok'=>false,'message'=>'Email/Client ID لازم است.'],400);
    try { api_out(['ok'=>true,'client'=>bluegate_provider_client($id,$external),'usage'=>bluegate_provider_usage($id,$external),'subscription'=>bluegate_provider_subscription($id,$external,['sub_id'=>(string)($input['sub_id']??'')])]); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'دریافت Client ناموفق بود: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_client_create') {
    require_admin($user); $id=(int)($input['provider_id']??0);
    try { $result=bluegate_provider_create_client($id,$input); log_admin_action((int)$user['telegram_id'],'provider_client_create','service_provider',$id,(string)($result['external_username']??'')); api_out(['ok'=>true,'result'=>$result]); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ساخت Client ناموفق بود: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_client_update') {
    require_admin($user); $id=(int)($input['provider_id']??0); $external=trim((string)($input['external_id']??$input['email']??''));
    if($external==='') api_out(['ok'=>false,'message'=>'Email/Client ID لازم است.'],400);
    foreach(['traffic_limit_bytes','expires_at','external_client_id','inbound_id','flow','limit_ip','limit_hwid'] as $k){ if(array_key_exists($k,$input) && trim((string)$input[$k])==='') unset($input[$k]); }
    try { $result=bluegate_provider_update_client($id,$external,$input); log_admin_action((int)$user['telegram_id'],'provider_client_update','service_provider',$id,$external); api_out(['ok'=>true,'result'=>$result]); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'بروزرسانی Client ناموفق بود: '.api_exception_code($e)],400); }
}
if ($action === 'admin_provider_client_delete') {
    require_admin($user); $id=(int)($input['provider_id']??0); $external=trim((string)($input['external_id']??$input['email']??''));
    if($external==='') api_out(['ok'=>false,'message'=>'Email/Client ID لازم است.'],400);
    try { bluegate_provider_delete_client($id,$external); log_admin_action((int)$user['telegram_id'],'provider_client_delete','service_provider',$id,$external); api_out(['ok'=>true,'message'=>'Client حذف شد.']); }
    catch(Throwable $e){ api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'حذف Client ناموفق بود: '.api_exception_code($e)],400); }
}

if ($action === 'admin_service_plan_addons') { require_admin($user);try{$pid=(int)($input['service_plan_id']??0);if($pid<=0)throw new RuntimeException('PLAN_REQUIRED');$q=db()->prepare('UPDATE service_plans SET extra_traffic_price_per_gb=?,extra_day_price=?,addon_min_gb=?,addon_max_gb=?,addon_min_days=?,addon_max_days=? WHERE id=?');$q->execute([max(0,(int)($input['extra_traffic_price_per_gb']??0)),max(0,(int)($input['extra_day_price']??0)),max(1,(int)($input['addon_min_gb']??1)),max(1,(int)($input['addon_max_gb']??500)),max(1,(int)($input['addon_min_days']??1)),max(1,(int)($input['addon_max_days']??365)),$pid]);log_admin_action((int)$user['telegram_id'],'service_plan_addons','service_plan',$pid);api_out(['ok'=>true,'catalog'=>catalog_public_payload()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'تنظیم قیمت افزونه ناموفق بود.'],400);} }
if ($action === 'admin_plan_provider_maps') { require_admin($user); api_out(['ok'=>true,'maps'=>bluegate_plan_provider_maps(!empty($input['service_plan_id'])?(int)$input['service_plan_id']:null),'jobs'=>bluegate_provisioning_jobs(60),'infrastructure'=>bluegate_infrastructure_summary()]); }
if ($action === 'admin_plan_provider_map_save') { require_admin($user); try{$id=bluegate_save_plan_provider_map($input);log_admin_action((int)$user['telegram_id'],'plan_provider_map_save','service_plan_provider_map',$id);api_out(['ok'=>true,'map_id'=>$id,'maps'=>bluegate_plan_provider_maps(),'infrastructure'=>bluegate_infrastructure_summary()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ذخیره Mapping ناموفق بود: '.api_exception_code($e)],400);} }
if ($action === 'admin_plan_provider_map_delete') { require_admin($user);$id=(int)($input['map_id']??0);bluegate_delete_plan_provider_map($id);log_admin_action((int)$user['telegram_id'],'plan_provider_map_delete','service_plan_provider_map',$id);api_out(['ok'=>true,'maps'=>bluegate_plan_provider_maps(),'infrastructure'=>bluegate_infrastructure_summary()]); }
if ($action === 'admin_provisioning_run') { require_admin($user);$r=bluegate_run_due_provisioning_jobs((int)($input['limit']??20));api_out(['ok'=>true,'result'=>$r,'jobs'=>bluegate_provisioning_jobs(60),'infrastructure'=>bluegate_infrastructure_summary()]); }
if ($action === 'admin_provisioning_retry') { require_admin($user);try{$r=bluegate_retry_provisioning_job((int)($input['job_id']??0),true);api_out(['ok'=>true,'result'=>$r,'jobs'=>bluegate_provisioning_jobs(60),'infrastructure'=>bluegate_infrastructure_summary()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Retry ناموفق بود: '.api_exception_code($e)],400);} }
if ($action === 'admin_order_provision_now') { require_admin_perm($user,'orders');try{$oid=(int)($input['order_id']??0);$r=bluegate_admin_provision_order_now($oid);$fresh=$r['order']??order_by_id($oid);$msg=($r['state']??'')==='delivered'?'Client ساخته شد و لینک Subscription برای مشتری ارسال شد.':'ساخت سرویس شروع شد و در صف Retry خودکار قرار دارد.';api_out(['ok'=>true,'message'=>$msg,'result'=>$r,'order'=>$fresh?order_public_payload($fresh,true):null,'jobs'=>bluegate_provisioning_jobs(60),'infrastructure'=>bluegate_infrastructure_summary()]);}catch(Throwable $e){$code=api_exception_code($e);$raw=$e->getMessage();$msg=$code==='NO_AUTOMATED_MAPPING'?'برای این پلن هنوز پنل/Inbound مپ نشده است.':($code==='ORDER_NOT_PAID'?'اول پرداخت سفارش را تأیید کن.':('ساخت سرویس از پنل ناموفق بود: '.preg_replace('/^PROVISIONING_FAILED:\s*/','',$raw)));api_out(['ok'=>false,'error'=>$code,'message'=>$msg],400);} }
if ($action === 'admin_routing_settings') { require_admin($user);try{$r=bluegate_save_routing_settings($input);log_admin_action((int)$user['telegram_id'],'routing_settings','system',1);api_out(['ok'=>true,'settings'=>$r,'routing'=>bluegate_routing_dashboard(),'infrastructure'=>bluegate_infrastructure_summary()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Saving routing settings failed.'],400);} }
if ($action === 'admin_monitoring_run') { require_admin($user);try{$r=bluegate_run_monitoring((int)($input['limit']??0));api_out(['ok'=>true,'result'=>$r,'monitoring'=>bluegate_monitoring_dashboard(),'infrastructure'=>bluegate_infrastructure_summary()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Monitoring run failed.'],500);} }
if ($action === 'admin_monitoring_settings') { require_admin($user);try{$r=bluegate_save_monitoring_settings($input);log_admin_action((int)$user['telegram_id'],'monitoring_settings','system',1);api_out(['ok'=>true,'settings'=>$r,'monitoring'=>bluegate_monitoring_dashboard()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Saving monitoring settings failed.'],400);} }
...
if ($action === 'admin_notification_broadcast') {
    require_admin($user);
    $title=trim((string)($input['title']??''));$body=trim((string)($input['body']??''));
    if($title==='')api_out(['ok'=>false,'error'=>'TITLE_REQUIRED','message'=>'عنوان اعلان الزامی است.'],400);
    $type=trim((string)($input['type']??'info'));$audience=trim((string)($input['audience']??'all'));
    $actionType=trim((string)($input['action_type']??''));$actionValue=trim((string)($input['action_value']??''));
    if($actionType==='url'&&$actionValue!==''&&!preg_match('#^https://#i',$actionValue))api_out(['ok'=>false,'error'=>'INVALID_URL','message'=>'لینک اعلان باید با https:// شروع شود.'],400);
    try{$campaign=create_notification_campaign((int)$user['telegram_id'],$type,$title,$body,$actionType,$actionValue,$audience);log_admin_action((int)$user['telegram_id'],'notification_broadcast','notification_campaign',(int)$campaign['id'],$campaign['recipient_count'].' recipients');api_out(admin_payload()+['notification_campaign'=>$campaign,'message'=>'اعلان برای '.number_format((int)$campaign['recipient_count']).' کاربر ثبت شد.']);}
    catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'ارسال اعلان انجام نشد.'],500);}
}
...
if ($action === 'admin_crypto_wallet_save') {
    ...
    $id=max(0,(int)($input['wallet_id']??0));
    $title=trim((string)($input['title']??''));
    $network=strtoupper(trim((string)($input['network']??'')));
    $asset=strtoupper(trim((string)($input['asset']??'')));
    $address=trim((string)($input['address']??''));
    $rate=strtoupper(trim((string)($input['rate_symbol']??$asset)));
    $active=bool_input($input['is_active']??1)?1:0;
    $sort=(int)($input['sort_order']??99);
    $minConfirm=max(1,min(50,(int)($input['min_confirmations']??1)));
    if($asset===''||$network===''||$address==='') api_out(['ok'=>false,'message'=>'ارز، شبکه و آدرس Wallet الزامی است.'],400);
    if(!crypto_wallet_supported($asset,$network)) api_out(['ok'=>false,'message'=>'این ترکیب ارز/شبکه در BlueGate بررسی خودکار ندارد. از یکی از گزینه‌های پشتیبانی‌شده استفاده کنید.'],400);
    if($title==='') $title=$asset.' '.$network;
    $dup=db()->prepare('SELECT id FROM crypto_wallets WHERE UPPER(asset)=? AND UPPER(network)=? AND address=? AND id<>? LIMIT 1');
    $dup->execute([$asset,$network,$address,$id]);
    if($dup->fetch()) api_out(['ok'=>false,'message'=>'این Wallet با همین ارز، شبکه و آدرس از قبل ثبت شده است.'],409);
    if($id>0){
        $q=db()->prepare('UPDATE crypto_wallets SET title=?,network=?,asset=?,address=?,rate_symbol=?,is_active=?,sort_order=?,min_confirmations=? WHERE id=?');
        $q->execute([$title,$network,$asset,$address,$rate?:$asset,$active,$sort,$minConfirm,$id]);
        if(!$q->rowCount() && !crypto_wallet_by_id($id)) api_out(['ok'=>false,'message'=>'Wallet پیدا نشد.'],404);
    }else{
        $q=db()->prepare('INSERT INTO crypto_wallets (title,network,asset,address,rate_symbol,is_active,sort_order,min_confirmations) VALUES (?,?,?,?,?,?,?,?)');
        $q->execute([$title,$network,$asset,$address,$rate?:$asset,$active,$sort,$minConfirm]);
        $id=(int)db()->lastInsertId();
    }
    log_admin_action((int)$user['telegram_id'],'crypto_wallet_save','crypto_wallet',$id,$asset.' '.$network);
    api_out(['ok'=>true,'wallet'=>crypto_wallet_admin_payload(crypto_wallet_by_id($id)),'wallets'=>array_values(array_map('crypto_wallet_admin_payload',crypto_wallets(false))),'crypto_wallets_text'=>crypto_wallets_lines(),'message'=>'Wallet ذخیره شد.']);
}
if ($action === 'admin_crypto_wallet_delete') {
    require_admin($user);
    $id=max(0,(int)($input['wallet_id']??0));
    $w=crypto_wallet_by_id($id);if(!$w)api_out(['ok'=>false,'message'=>'Wallet پیدا نشد.'],404);
    $q=db()->prepare('DELETE FROM crypto_wallets WHERE id=?');$q->execute([$id]);
    log_admin_action((int)$user['telegram_id'],'crypto_wallet_delete','crypto_wallet',$id,(string)($w['asset']??''));
    api_out(['ok'=>true,'wallets'=>array_values(array_map('crypto_wallet_admin_payload',crypto_wallets(false))),'crypto_wallets_text'=>crypto_wallets_lines(),'message'=>'Wallet حذف شد.']);
}
if ($action === 'admin_save_settings') {
    ...
    if(isset($input['stars_rate_manual_toman'])) set_setting('stars_rate_toman', max(1,(int)$input['stars_rate_manual_toman']));
    if(isset($input['stars_rate_mode'])) { $m=strtolower((string)$input['stars_rate_mode']); set_setting('stars_rate_mode', in_array($m,['auto','manual'],true)?$m:'auto'); }
    if(isset($input['stars_base_usdt'])) set_setting('stars_base_usdt',(string)max(0.000001,(float)$input['stars_base_usdt']));
    if(isset($input['stars_rate_markup_percent'])) set_setting('stars_rate_markup_percent',(string)max(0,(float)$input['stars_rate_markup_percent']));
...
if ($action === 'get_usdt_rate' || $action === 'get_crypto_rates') {
    ...
    api_out([
        ...
        'rates' => [
            ...
            'STARS'=> ['rate'=>(float)$starsMeta['rate'],'source'=>(string)$starsMeta['source'],'updated_at'=>$starsMeta['updated_at'],'is_live'=>!empty($starsMeta['is_live']),'base_usdt'=>(float)$starsMeta['base_usdt'],'markup_percent'=>(float)$starsMeta['markup_percent']],
        ]
    ]);
}
...
/* BlueGate Phase 10 API */
if ($action === 'trial_request') { try{api_out(['ok'=>true,'service'=>bluegate_request_trial((int)$user['id']),'trial'=>bluegate_trial_eligibility((int)$user['id'])]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Trial could not be created.'],400);} }
if ($action === 'support_ticket_create') { try{$r=bluegate_create_ticket((int)$user['id'],$input);api_out(['ok'=>true]+$r+['tickets'=>bluegate_user_tickets((int)$user['id'])]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e),'message'=>'Ticket could not be created.'],400);} }
if ($action === 'support_ticket_messages') { $tid=(int)($input['ticket_id']??0);$q=db()->prepare('SELECT id FROM support_tickets WHERE id=? AND user_id=?');$q->execute([$tid,(int)$user['id']]);if(!$q->fetchColumn())api_out(['ok'=>false,'error'=>'TICKET_NOT_FOUND'],404);api_out(['ok'=>true,'messages'=>bluegate_ticket_messages($tid)]); }
if ($action === 'support_ticket_reply') { try{$tid=(int)($input['ticket_id']??0);$q=db()->prepare('SELECT id FROM support_tickets WHERE id=? AND user_id=?');$q->execute([$tid,(int)$user['id']]);if(!$q->fetchColumn())throw new RuntimeException('TICKET_NOT_FOUND');bluegate_reply_ticket($tid,'user',(int)$user['id'],(string)($input['message']??''));api_out(['ok'=>true,'messages'=>bluegate_ticket_messages($tid)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],400);} }
if ($action === 'service_usage_history') { try{api_out(['ok'=>true,'history'=>bluegate_service_usage_history((int)($input['service_id']??0),(int)$user['id'],(int)($input['days']??30))]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],404);} }
if ($action === 'service_event_history') { try{api_out(['ok'=>true,'events'=>bluegate_service_event_history((int)($input['service_id']??0),(int)$user['id'],150)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],404);} }
if ($action === 'admin_advanced_summary') { require_admin($user);api_out(['ok'=>true,'advanced'=>bluegate_advanced_dashboard()]); }
if ($action === 'admin_trial_settings') { require_admin($user);try{api_out(['ok'=>true,'settings'=>bluegate_save_trial_settings($input),'advanced'=>bluegate_advanced_dashboard()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],400);} }
if ($action === 'admin_provider_economics') { require_admin($user);try{api_out(['ok'=>true,'economics'=>bluegate_save_provider_economics($input),'advanced'=>bluegate_advanced_dashboard()]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],400);} }
if ($action === 'admin_support_tickets') { require_admin($user);$rows=db()->query('SELECT t.*,u.telegram_id,u.username FROM support_tickets t JOIN users u ON u.id=t.user_id ORDER BY FIELD(t.status,"open","waiting_admin","waiting_customer","resolved","closed"),t.last_message_at DESC LIMIT 200')->fetchAll()?:[];api_out(['ok'=>true,'tickets'=>$rows]); }
if ($action === 'admin_support_ticket_messages') { require_admin($user);api_out(['ok'=>true,'messages'=>bluegate_ticket_messages((int)($input['ticket_id']??0))]); }
if ($action === 'admin_support_ticket_reply') { require_admin($user);try{$tid=(int)($input['ticket_id']??0);bluegate_reply_ticket($tid,'admin',(int)$user['id'],(string)($input['message']??''));api_out(['ok'=>true,'messages'=>bluegate_ticket_messages($tid)]);}catch(Throwable $e){api_out(['ok'=>false,'error'=>api_exception_code($e)],400);} }
if ($action === 'admin_support_ticket_status') { require_admin($user);$tid=(int)($input['ticket_id']??0);$st=(string)($input['status']??'open');if(!in_array($st,['open','waiting_admin','waiting_customer','resolved','closed'],true))$st='open';db()->prepare('UPDATE support_tickets SET status=?,closed_at=IF(? IN ("closed","resolved"),NOW(),NULL) WHERE id=?')->execute([$st,$st,$tid]);api_out(['ok'=>true]); }
