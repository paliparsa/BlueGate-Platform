...
if ($action === 'admin_crypto_wallet_save') {
    ...
    if($id>0){
        $q=db()->prepare('UPDATE crypto_wallets SET title=?,network=?,asset=?,address=?,rate_symbol=?,is_active=?,sort_order=? WHERE id=?');
        $q->execute([$title,$network,$asset,$address,$rate?:$asset,$active,$sort,$id]);
        ...
        $q->execute([$title,$network,$asset,$address,$rate?:$asset,$active,$sort]);
        $id=(int)db()->lastInsertId();
    }
    log_admin_action((int)$user['telegram_id'],'crypto_wallet_save','crypto_wallet',$id,$asset.' '.$network);
    api_out(['ok'=>true,'wallet'=>crypto_wallet_by_id($id),'wallets'=>array_values(crypto_wallets(false)),'crypto_wallets_text'=>crypto_wallets_lines(),'message'=>'Wallet ذخیره شد.']);
}
if ($action === 'admin_crypto_wallet_delete') {
    ...
    api_out(['ok'=>true,'wallets'=>array_values(crypto_wallets(false)),'crypto_wallets_text'=>crypto_wallets_lines(),'message'=>'Wallet حذف شد.']);
}
