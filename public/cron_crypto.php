...
try {
    ...
    // BlueGate 3.x -> 4.x compatibility bridge.
    // The first update can still be running under the old CLI, whose cron file only
    // invokes cron_crypto.php. Until the new dedicated cron entries are installed,
    // keep provisioning and monitoring alive from this legacy one-minute job.
    $serviceEngineCompat=['provisioning'=>['skipped'=>true],'monitoring'=>['skipped'=>true]];
    if (($mode === 'check_payments' || $mode === 'all') && function_exists('bluegate_run_due_provisioning_jobs')) {
        $cacheDir=__DIR__.'/../storage/cache'; if(!is_dir($cacheDir)) @mkdir($cacheDir,0750,true);
        $provBeat=$cacheDir.'/cron-provisioning.last';
        $provFresh=is_file($provBeat) && (time()-(int)@file_get_contents($provBeat) < 100);
        if(!$provFresh){
            try{$serviceEngineCompat['provisioning']=bluegate_run_due_provisioning_jobs(20);@file_put_contents($provBeat,(string)time());}
            catch(Throwable $se){$serviceEngineCompat['provisioning']=['ok'=>false,'error'=>$se->getMessage()];error_log('[BlueGate Service Cron Compat] '.$se->getMessage());}
        }
        $monBeat=$cacheDir.'/cron-monitoring.last';
        $monFresh=is_file($monBeat) && (time()-(int)@file_get_contents($monBeat) < 420);
        if(!$monFresh && function_exists('bluegate_run_monitoring')){
            try{$serviceEngineCompat['monitoring']=bluegate_run_monitoring(0);@file_put_contents($monBeat,(string)time());}
            catch(Throwable $me){$serviceEngineCompat['monitoring']=['ok'=>false,'error'=>$me->getMessage()];error_log('[BlueGate Monitoring Cron Compat] '.$me->getMessage());}
        }
    }
    ...
    echo json_encode([
        ...
        'service_engine_compat'=>$serviceEngineCompat,
