...
function plans(){if(!current.variants.length)return '';return `<section class="purchase-plan-step"><div class="purchase-step-head"><div><span>انتخاب پلن</span><h4>پلن مناسب رو انتخاب کن</h4></div><b>${current.variants.length} گزینه</b></div><div class="purchase-plan-grid">${current.variants.map(v=>{const orig=originalPrice(v);return `<button type="button" class="purchase-plan-option ${v.id===Number(current.selectedId)?'active':''}" data-mini-purchase-variant="${v.id}"><span class="purchase-plan-title">${esc(v.title)}</span><div class="purchase-plan-price">${orig?`<s>${fmt(orig)}</s>`:''}<strong>${fmt(v.price)} <small>تومان</small></strong>${v.discount_percent?`<em class="purchase-plan-discount">${fmt(v.discount_percent)}٪ تخفیف</em>`:''}</div><i>✓</i></button>`}).join('')}</div></section>`}
function render(){
