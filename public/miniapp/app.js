...
/* ===== v3.0.7.5: single-active-layer navigation guard ===== */
...
function closeMiniTransientLayers(except=''){
  MINI_LAYER_IDS.forEach(id=>{if(id!==except&&miniLayerIsOpen(id))closeMiniLayerById(id)});
  document.documentElement.classList.toggle('sheet-open',miniLayerIsOpen('appSheet'));
  document.documentElement.classList.toggle('purchase-confirm-open',miniLayerIsOpen('miniPurchaseOverlay'));
  document.body.classList.toggle('purchase-confirm-open',miniLayerIsOpen('miniPurchaseOverlay'));
}
function topMiniLayer(){for(let i=MINI_LAYER_IDS.length-1;i>=0;i--){if(miniLayerIsOpen(MINI_LAYER_IDS[i]))return MINI_LAYER_IDS[i]}return null}
function closeTopMiniLayer(){const id=topMiniLayer();return id?closeMiniLayerById(id):false}
document.addEventListener('bluegate:before-layer-open',e=>closeMiniTransientLayers(e.detail?.id||''));
function prepareMiniLayer(id){closeMiniTransientLayers(id);window.BlueGateUI?.beforeOpen?.(id)}
...
try{tg?.BackButton?.onClick?.(()=>{if(closeTopMiniLayer()){syncTelegramBackButton();return}if(currentServiceOrderId){currentServiceOrderId=null;currentTab='home';renderUser();return}if(currentOrderId){currentOrderId=null;currentTab='orders';renderUser();return}if(currentTab!=='shop'){currentTab='shop';renderUser();return}})}catch(_){}
...
function setAdminTab(tab){if(['products','categories','variants'].includes(tab))tab='catalog';if(!['dashboard','catalog','orders','more','notifications','inventory','coupons','settings','activity','roles','backups'].includes(tab))tab='dashboard';currentAdminTab=tab;saveAppLastState();renderAdmin()}
...
function runMiniNotificationAction(n){const a=String(n.action_type||'');const v=String(n.action_value||'');BlueGateUI.closeSheet();if(n.order_id||a==='orders'){currentTab='orders';currentOrderId=Number(n.order_id||v||0)||null;renderUser();return}if(a==='store'){currentTab='shop';renderUser();return}if(a==='wallet'){currentTab='wallet';renderUser();return}if(a==='account'||a==='profile'){currentTab='home';renderUser();return}if(a==='service'&&v){currentServiceOrderId=Number(v)||null;currentTab='home';renderUser();return}if(a==='url'&&/^https:\/\//i.test(v)){if(TG?.openLink)TG.openLink(v);else window.open(v,'_blank','noopener')}}
...
let miniFastState={type:'direct',groups:[]};
function miniFastDefaults(type='direct'){const map={direct:[{name:'Plans',plans:[['1 Month',120000],['3 Months',300000]]}],vpn:[{name:'Standard',plans:[['30GB',149000],['50GB',249000],['100GB',499000]]},{name:'Pro',plans:[['30GB',249000],['50GB',349000]]}],account:[{name:'Plans',plans:[['1 Month',120000],['3 Months',300000],['12 Months',890000]]}],code:[{name:'Plans',plans:[['10 USD',100000],['25 USD',240000],['50 USD',460000]]}]};return JSON.parse(JSON.stringify(map[type]||map.direct))}
function miniFastSerialize(){return miniFastState.groups.map(g=>`${g.name}: ${g.plans.map(p=>`${p[0]}=${Number(p[1]||0)}`).join(', ')}`).join('\n')}
function drawMiniFast(){const box=$('miniFastPreview'),ta=$('mcf_groups');if(!box||!ta)return;box.innerHTML=miniFastState.groups.map((g,gi)=>`<article class="mini-fast-group-card"><header><div><b>${esc(g.name==='Plans'?'پلن‌های مستقیم':g.name)}</b><small>${nf(g.plans.length)} پلن</small></div><button type="button" data-mini-fast-add="${gi}">＋ پلن</button></header><div class="mini-fast-plan-grid">${g.plans.map((p,pi)=>`<button type="button" class="mini-fast-plan-card" data-mini-fast-edit="${gi}:${pi}"><span><b>${esc(p[0])}</b><small>${fmt(p[1])}</small></span><i>ویرایش</i></button>`).join('')}</div></article>`).join('');ta.value=miniFastSerialize()}
function miniFastPlanPopup(gi,pi=-1){const g=miniFastState.groups[gi];if(!g)return;const old=pi>=0?g.plans[pi]:['',0];let h=$('miniFastPlanPopup');if(!h){h=document.createElement('div');h.id='miniFastPlanPopup';h.className='mini-fast-plan-popup';document.body.appendChild(h)}h.innerHTML=`<div class="mini-fast-plan-backdrop" data-mini-fast-plan-close></div><section><header><div><small>QUICK PLAN</small><h3>${pi>=0?'ویرایش پلن':'افزودن پلن'}</h3></div><button data-mini-fast-plan-close>×</button></header><label>عنوان پلن<input id="mfpTitle" value="${esc(old[0]||'')}" placeholder="مثلاً 1 Month"></label><label>قیمت تومان<input id="mfpPrice" type="number" inputmode="numeric" value="${Number(old[1]||0)}"></label><footer><button data-mini-fast-plan-close>انصراف</button>${pi>=0?`<button class="danger" id="mfpDelete">حذف</button>`:''}<button class="primary" id="mfpSave">ذخیره</button></footer></section>`;h.classList.add('open');const close=()=>h.classList.remove('open');h.querySelectorAll('[data-mini-fast-plan-close]').forEach(x=>x.onclick=close);h.querySelector('#mfpSave').onclick=()=>{const title=$('mfpTitle').value.trim(),price=Number($('mfpPrice').value||0);if(!title||price<=0)return showStatus('عنوان و قیمت پلن را کامل کن','error');if(pi>=0)g.plans[pi]=[title,price];else g.plans.push([title,price]);close();drawMiniFast()};h.querySelector('#mfpDelete')?.addEventListener('click',()=>{g.plans.splice(pi,1);close();drawMiniFast()})}
function openCatalogFast(){miniFastState={type:'direct',groups:miniFastDefaults('direct')};openEdit('ساخت سریع سرویس',[{html:`<div class="mini-fast-builder"><div class="mcw-copy"><small>QUICK CREATE</small><h3>نوع سرویس را انتخاب کن</h3><p class="muted">پریست را انتخاب کن و پلن‌ها را با کارت ویرایش کن.</p></div><div class="mini-fast-presets"><button type="button" class="active" data-mini-fast="direct"><span>⚡</span><b>پلن مستقیم</b><small>اشتراک زمانی</small></button><button type="button" data-mini-fast="vpn"><span>🌐</span><b>VPN</b><small>Standard / Pro</small></button><button type="button" data-mini-fast="account"><span>🔑</span><b>اکانت</b><small>ماهانه / سالانه</small></button><button type="button" data-mini-fast="code"><span>🎟️</span><b>کد / گیفت</b><small>مقادیر مختلف</small></button></div></div>`},{title:'اطلاعات سرویس',fields:[{id:'mcf_name',label:'نام سرویس',placeholder:'مثلاً BluePing'},{id:'mcf_cat',label:'دسته فروشگاه',type:'select',options:catalogCategoryOptions()},{id:'mcf_desc',label:'توضیح کوتاه',type:'textarea',placeholder:'توضیح سرویس'}]},{html:`<div class="mini-fast-preview" id="miniFastPreview"></div><textarea id="mcf_groups" class="hidden" aria-hidden="true"></textarea>`}],async()=>adminAction('admin_catalog_fast_create',{service_name:val('mcf_name'),category_id:val('mcf_cat'),description:val('mcf_desc'),groups_text:miniFastSerialize()}));setTimeout(()=>{document.querySelectorAll('[data-mini-fast]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-mini-fast]').forEach(x=>x.classList.toggle('active',x===b));miniFastState.type=b.dataset.miniFast;miniFastState.groups=miniFastDefaults(miniFastState.type);drawMiniFast()}));const box=$('miniFastPreview');box?.addEventListener('click',e=>{const add=e.target.closest('[data-mini-fast-add]'),edit=e.target.closest('[data-mini-fast-edit]');if(add)return miniFastPlanPopup(Number(add.dataset.miniFastAdd),-1);if(edit){const [g,p]=edit.dataset.miniFastEdit.split(':').map(Number);return miniFastPlanPopup(g,p)}});drawMiniFast()},0)}
