...
function renderAdminCatalog(){const ca=catalogAdminData(),pv=ca.preview||{},c=pv.counts||{},tree=ca.tree||[],enabled=!!ca.public?.enabled;const badge=x=>x==='high'?'<span class="chip-mini chip-active">🟢 قطعی</span>':x==='medium'?'<span class="chip-mini chip-featured">🟡 احتمالاً درست</span>':'<span class="chip-mini chip-off">🔴 نیازمند بررسی</span>';return `<article class="admin-card catalog-mobile-hero"><div class="admin-card-head"><span class="admin-card-icon">🧭</span><div><h3>کاتالوگ v2</h3><p class="muted">Category → Service → Group → Plan</p></div></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(c.services||0)}</b><span>سرویس</span></div><div class="dq-stat"><b>${nf(c.groups||0)}</b><span>زیرسرویس</span></div><div class="dq-stat"><b>${nf(c.plans||0)}</b><span>پلن</span></div><div class="dq-stat"><b>${nf(c.review||0)}</b><span>بررسی</span></div></div><p class="muted">${enabled?'✅ Storefront از کاتالوگ جدید پیروی می‌کند.':'فعلاً فقط Preview است؛ هیچ دیتای قدیمی حذف یا جابه‌جا نمی‌شود.'}</p><div class="catalog-mobile-actions"><button class="secondary" data-catalog-preview>🔍 اسکن مجدد</button><button class="secondary" data-catalog-fast>⚡ ساخت سریع</button>${!enabled?'<button class="primary" data-catalog-apply>✨ اعمال Migration</button>':''}</div></article><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">✨</span><div><h3>Auto Organizer</h3><p class="muted">پیشنهاد سیستم قبل از اعمال</p></div></div><div class="catalog-mobile-preview">${(pv.proposals||[]).map(x=>`<div class="catalog-mobile-preview-row"><div><b>${esc(x.service_name)}</b><small>${esc(x.category_title||'سایر')} · ${nf((x.groups||[]).length)} گروه · ${nf(x.plan_count||0)} پلن</small></div><div class="catalog-mobile-review-action">${badge(x.confidence)}${x.mapped?'<span class="chip-mini chip-active">✓ نگاشت</span>':(x.confidence==='review'?`<button class="secondary" data-catalog-apply-one="${x.legacy_product_id}">نگاشت</button>`:'')}</div></div>`).join('')||'<p class="muted">هنوز چیزی اسکن نشده.</p>'}</div></article><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🛠</span><div><h3>مرتب‌سازی دستی</h3><p class="muted">نسخه موبایل با انتخاب‌گر ساده</p></div></div><div class="catalog-mobile-actions"><button class="secondary" data-catalog-move-group>انتقال زیرسرویس</button><button class="secondary" data-catalog-move-plan>انتقال پلن</button><button class="secondary" data-catalog-add-service>+ سرویس</button><button class="secondary" data-catalog-add-group>+ زیرسرویس</button><button class="secondary" data-catalog-add-plan>+ پلن</button></div></article><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🌳</span><div><h3>ساختار فعلی</h3><p class="muted">Default Group برای مشتری نمایش داده نمی‌شود.</p></div></div><div class="catalog-mobile-tree">${tree.map(s=>`<details class="catalog-mobile-service" open><summary>${esc(s.category_emoji||'🛍️')} <b>${esc(s.name)}</b><span>${nf((s.groups||[]).length)} گروه</span></summary>${(s.groups||[]).map(g=>`<div class="catalog-mobile-group"><b>${g.is_default?'پلن‌های مستقیم':esc(g.name)}</b>${g.is_default?'<small class="muted">Default مخفی</small>':''}<div>${(g.plans||[]).map(p=>`<span>${esc(p.title)} · ${fmt(p.price)} ت</span>`).join('')||'<span class="muted">بدون پلن</span>'}</div></div>`).join('')}</details>`).join('')||'<p class="muted">بعد از اعمال Migration ساختار اینجا دیده می‌شود.</p>'}</div></article>`}
function openCatalogFast(){openEdit('ساخت سریع کاتالوگ',[{title:'سرویس و ساختار',fields:[{id:'cf_service',label:'نام سرویس'},{id:'cf_cat',label:'دسته',type:'select',options:catalogCategoryOptions()},{id:'cf_desc',label:'توضیح',type:'textarea'},{id:'cf_groups',label:'ساختار سریع',type:'textarea',placeholder:'Standard: 20GB=99000, 30GB=149000\nPro: 10GB=149000, 20GB=249000'}]}],async()=>adminAction('admin_catalog_fast_create',{service_name:val('cf_service'),category_id:val('cf_cat'),description:val('cf_desc'),groups_text:val('cf_groups')}))}
function openCatalogMoveGroup(){openEdit('انتقال زیرسرویس',[{title:'مقصد جدید',fields:[{id:'cmg_group',label:'زیرسرویس',type:'select',options:catalogGroupOptions()},{id:'cmg_service',label:'سرویس مقصد',type:'select',options:catalogServiceOptions()}]}],async()=>adminAction('admin_catalog_move_group',{group_id:val('cmg_group'),service_id:val('cmg_service')}))}
function openCatalogMovePlan(){openEdit('انتقال پلن',[{title:'مقصد جدید',fields:[{id:'cmp_plan',label:'پلن',type:'select',options:catalogPlanOptions()},{id:'cmp_group',label:'زیرسرویس مقصد',type:'select',options:catalogGroupOptions()}]}],async()=>adminAction('admin_catalog_move_plan',{plan_id:val('cmp_plan'),group_id:val('cmp_group')}))}
function openCatalogAddService(){openEdit('سرویس جدید',[{title:'اطلاعات سرویس',fields:[{id:'cas_name',label:'نام سرویس'},{id:'cas_cat',label:'دسته',type:'select',options:catalogCategoryOptions()},{id:'cas_badge',label:'Badge'},{id:'cas_theme',label:'Theme',value:'blue'},{id:'cas_desc',label:'توضیح',type:'textarea'}]}],async()=>adminAction('admin_catalog_add_service',{name:val('cas_name'),category_id:val('cas_cat'),badge:val('cas_badge'),theme:val('cas_theme'),description:val('cas_desc')}))}
function openCatalogAddGroup(){openEdit('زیرسرویس جدید',[{title:'اطلاعات زیرسرویس',fields:[{id:'cag_service',label:'سرویس',type:'select',options:catalogServiceOptions()},{id:'cag_name',label:'نام زیرسرویس'},{id:'cag_desc',label:'توضیح',type:'textarea'}]}],async()=>adminAction('admin_catalog_add_group',{service_id:val('cag_service'),name:val('cag_name'),description:val('cag_desc')}))}
function openCatalogAddPlan(){openEdit('پلن جدید',[{title:'اطلاعات پلن',fields:[{id:'cap_group',label:'زیرسرویس',type:'select',options:catalogGroupOptions()},{id:'cap_title',label:'عنوان پلن'},{id:'cap_price',label:'قیمت تومان',type:'number',value:0},{id:'cap_days',label:'مدت روز',type:'number',value:0},{id:'cap_desc',label:'توضیح',type:'textarea'}]}],async()=>adminAction('admin_catalog_add_plan',{group_id:val('cap_group'),title:val('cap_title'),price:val('cap_price'),duration_days:val('cap_days'),description:val('cap_desc')}))}
