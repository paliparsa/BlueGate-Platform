...
async function cartCheckout(){if(!_cart.length)return;if(!await BlueGateUI.confirm({title:'ثبت سبد خرید',message:`${nf(cartCount())} سفارش ساخته شود؟`,confirmText:'ثبت سفارش‌ها'}))return;const btn=$('cartCheckoutBtn');if(btn)btn.disabled=true;let ok=0,fail=0;const remaining=[];for(const item of _cart){let failedQty=0;const qty=Math.max(1,Number(item.qty||1));for(let i=0;i<qty;i++){try{await api('create_order',{product_id:item.pid,variant_id:item.vid||null,use_wallet:0});ok++}catch(e){fail++;failedQty++}}if(failedQty>0)remaining.push({...item,qty:failedQty})}_cart=remaining;saveCart();if(!_cart.length)closeCartSheet();if(btn)btn.disabled=false;showStatus(`${ok?'✅':'⚠️'} ${nf(ok)} سفارش ساخته شد${fail?` · ${nf(fail)} ناموفق و در سبد باقی ماند`:''}`);state=await api('me');applyTheme(state);if(ok>0){currentTab='orders';renderUser()}else{renderCartSheet&&renderCartSheet()}}
...
function renderCatalogMobileWizard(){
  ...
  BlueGateUI.openSheet({type:'fullscreen',eyebrow:'CATALOG STUDIO',title:w.id?'ویرایش سرویس':'سرویس جدید',subtitle:`مرحله ${mobileCatalogStep+1} از ۵`,body,footer,onOpen:(sheet)=>{
    ...
    sheet.querySelectorAll('[data-mc-upload]').forEach(inp=>inp.addEventListener('change',async()=>{if(!inp.files?.[0])return;try{mcCollect();showStatus('در حال آپلود تصویر…');const url=await mcUploadImage(inp.files[0]);const kind=inp.dataset.mcUpload,key=inp.dataset.key;if(kind==='service')w.image_url=url;else if(kind==='group'){const g=w.groups.find(x=>x.key===key);if(g)g.image_url=url}else if(kind==='plan'){const p=w.groups.flatMap(g=>g.plans||[]).find(x=>x.key===key);if(p)p.image_url=url}try{w._draft_schema=MOBILE_CATALOG_DRAFT_SCHEMA;localStorage.setItem(`bg_mcw_${w.id||'new'}`,JSON.stringify(w))}catch(_){}renderCatalogMobileWizard();showStatus('تصویر آپلود شد','success')}catch(e){showStatus(e.message||'آپلود تصویر ناموفق بود','error')}}));
