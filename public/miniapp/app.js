...
function renderAdminAdvanced(){
  ...
  return `<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🚀</span><div><h3>امکانات پیشرفته</h3><p class="muted">تست رایگان، اقتصاد پنل‌ها، تمدید سراسری، مانیتورینگ و Failover</p></div></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(A.trials_active||0)}</b><span>تست فعال</span></div><div class="dq-stat"><b>${nf(mon.stats?.open_incidents||0)}</b><span>Incident باز</span></div><div class="dq-stat"><b>${nf(pa.length)}</b><span>پنل</span></div><div class="dq-stat"><b>${nf(routing.stats?.open_circuits||0)}</b><span>Circuit باز</span></div></div><div class="dashboard-quick-actions"><button class="quick-action" data-adv-trial><span>🧪</span><b>تنظیم تست</b></button><button class="quick-action" data-adv-addons><span>➕</span><b>تمدید و افزونه</b></button><button class="quick-action" data-adv-monitor><span>♥</span><b>مانیتور الآن</b></button><button class="quick-action" data-adv-routing><span>🧭</span><b>Failover</b></button></div></article>
  ${pa.map(p=>`<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">📈</span><div><h3>${esc(p.name||'پنل')}</h3><p class="muted">${p.driver==='xui'?'3x-ui':esc(p.driver||'')} · ${nf(p.active_services||0)} سرویس · ${(Number(p.traffic_used_bytes||0)/1073741824).toFixed(1)} GB مصرف</p></div><span class="chip-mini ${p.maintenance_mode?'chip-off':'chip-active'}">${p.maintenance_mode?'تعمیرات':'فعال'}</span></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${esc(p.monthly_cost||0)}</b><span>هزینه ${esc(p.cost_currency||'USD')}</span></div><div class="dq-stat"><b>${nf(p.revenue_toman||0)}</b><span>درآمد</span></div><div class="dq-stat"><b>${nf(p.soft_capacity_percent||90)}%</b><span>ظرفیت نرم</span></div></div><button class="secondary wide" data-adv-economics="${p.id}">هزینه، ظرفیت و تعمیرات</button></article>`).join('')||'<article class="admin-card"><p class="muted">هنوز داده تحلیلی برای پنل‌ها وجود ندارد.</p></article>'}
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🚨</span><div><h3>Incidentهای باز</h3><p class="muted">خطاهای تاییدشده اتصال</p></div></div>${inc.slice(0,10).map(x=>`<div class="admin-list-row"><div><b>${esc(x.provider_name||'Provider')}</b><small>${esc(x.message||x.title||'')} · ${esc(x.severity||'')}</small></div><span>${esc(x.opened_at||'')}</span></div>`).join('')||'<p class="muted">مشکل بازی وجود ندارد.</p>'}</article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">⚙</span><div><h3>آخرین مانیتورینگ</h3><p class="muted">نتیجه اجرای اتوماسیون</p></div></div>${runs.slice(0,10).map(x=>`<div class="admin-list-row"><div><b>${esc(x.run_type||'monitor')} · ${esc(x.status||'')}</b><small>${nf(x.success_count||0)} موفق · ${nf(x.warning_count||0)} هشدار · ${nf(x.error_count||0)} خطا</small></div><span>${esc(x.started_at||x.created_at||'')}</span></div>`).join('')||'<p class="muted">هنوز اجرا نشده.</p>'}</article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🧭</span><div><h3>تصمیم‌های مسیریابی</h3><p class="muted">Provider انتخاب‌شده برای Failover</p></div></div>${decisions.slice(0,10).map(x=>`<div class="admin-list-row"><div><b>${esc(x.selected_provider_name||'بدون Provider')}</b><small>${esc(x.reason||x.strategy||'routing')}</small></div><span>#${nf(x.id)}</span></div>`).join('')||'<p class="muted">هنوز تصمیمی ثبت نشده.</p>'}</article>`;
}
