...
function miniTopupShell(title,subtitle,body){const o=ensureMiniCreditTopupSheet();o.innerHTML=`<div class="mini-credit-topup-backdrop" data-mini-topup-close></div><section class="mini-credit-topup-panel" role="dialog" aria-modal="true"><div class="mini-credit-sheet-handle"></div><header><div><small>BLUEGATE CREDIT</small><h3>${esc(title)}</h3><p>${esc(subtitle||'')}</p></div><button class="ghost" type="button" data-mini-topup-close>✕</button></header><div class="mini-credit-topup-body">${body}</div></section>`;o.classList.add('open');o.querySelectorAll('[data-mini-topup-close]').forEach(x=>x.addEventListener('click',closeMiniCreditTopup));return o}
