...
async function openSecureServiceViewer(orderId,title='مدیریت سرویس'){
  try{
    showStatus('در حال باز کردن سرویس…');
    const r=await api('service_viewer_ticket',{order_id:Number(orderId)});
    document.getElementById('miniSecureServiceViewer')?.remove();
    const ov=document.createElement('div');ov.id='miniSecureServiceViewer';ov.className='mini-service-viewer';
    ov.innerHTML=`<div class="mini-service-head"><button type="button" data-service-close>✕</button><div><b>🔒 ${esc(r.title||title)}</b><small>Secure Viewer</small></div><button type="button" data-service-refresh>↻</button></div><div class="mini-service-frame"><div class="mini-service-loading">در حال بارگذاری سرویس…</div><iframe src="${esc(r.viewer_url)}" sandbox="allow-forms allow-scripts allow-popups allow-downloads" referrerpolicy="no-referrer"></iframe></div>`;
    document.body.appendChild(ov);document.body.classList.add('mini-service-open');
    const fr=ov.querySelector('iframe'),ld=ov.querySelector('.mini-service-loading');fr.addEventListener('load',()=>ld?.classList.add('done'));
    const close=()=>{ov.remove();document.body.classList.remove('mini-service-open')};
    ov.querySelector('[data-service-close]').onclick=close;ov.querySelector('[data-service-refresh]').onclick=()=>{ld?.classList.remove('done');fr.src=fr.src};
...
function orderDetailHtml(o){
  ...
  return `<section class="detail-card order-detail-page order-detail-v2">
    ...
    <div class="actions sticky-actions od-sticky-actions">
      ${o.has_service_viewer ? `<button class="primary btn-v2 mini-service-open-btn" data-service-view="${o.id}" data-service-title="${esc(o.service_title||'مدیریت سرویس')}">🌐 باز کردن سرویس</button>` : ''}
