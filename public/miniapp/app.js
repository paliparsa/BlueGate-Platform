...
function openAdminActionSheet(type,id){
  ...
  const body=`<div class="order-control-center">
    ...
    <section class="oac-section"><header><span>👤</span><div><b>کاربر</b><small>پروفایل و ارتباط</small></div></header><div class="oac-action-grid">${o.user_id?`<button data-customer-360="${o.user_id}"><span>◎</span><b>پروفایل کاربر</b></button>`:''}${o.username?`<button data-chat-user="${esc(o.username)}"><span>✉</span><b>ارسال پیام</b></button>`:''}<button data-admin-order-note="${o.id}"><span>✎</span><b>یادداشت داخلی</b></button></div></section>
