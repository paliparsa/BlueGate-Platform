...
function mcStepHtml(){
  ...
  if(mobileCatalogStep===2){
    ...
    return `<div class="mcw-section-head"><span class="mcw-section-kicker">زیرسرویس‌ها</span><h3>گروه‌های سرویس</h3><p>تمام فیلدهای قابل ویرایش قبلی در کارت‌های جمع‌وجور نگه داشته شده‌اند.</p></div><div class="mcw-list">${w.groups.filter(g=>!g.is_default).map((g,i)=>`<section class="mcw-group mcw-group-clean" data-mcg="${g.key}">
      <div class="mcw-group-title"><span>${i+1}</span><div><b>${esc(g.name||'زیرسرویس جدید')}</b><small>${esc(g.slug||'بدون slug')}</small></div><button type="button" class="danger ghost" data-mc-remove-group="${g.key}">حذف</button></div>
      <div class="form-grid mcw-grid compact">
      ...
      <div class="mcw-inline-setting">${mcSettingRow('فعال','این زیرسرویس قابل انتخاب باشد','data-mcgf="is_active"',Number(g.is_active),'●')}</div>
    </section>`).join('')}</div><button class="secondary wide mcw-add-row" data-mc-add-group>＋ افزودن زیرسرویس</button>`;
  }
  ...
  if(mobileCatalogStep===3){
    ...
    return `<div class="mcw-section-head"><span class="mcw-section-kicker">پلن‌ها</span><h3>قیمت و نحوه تحویل</h3><p>تمام گزینه‌های تومان/USD، تخفیف، مدت، تحویل، پورسانت، تصویر و وضعیت فعال حفظ شده‌اند.</p></div><div class="mcw-plan-groups">${gs.map(g=>`<section class="mcw-plan-group-clean"><header><div><b>${g.is_default?'⚡ پلن‌های مستقیم':'🧩 '+esc(g.name)}</b><small>${nf((g.plans||[]).length)} پلن</small></div><button type="button" class="secondary" data-mc-add-plan="${g.key}">＋ پلن</button></header><div class="mcw-plan-list">${(g.plans||[]).map((p,idx)=>`<article class="mcw-plan mcw-plan-clean" data-mcp="${p.key}">
        <div class="mcw-plan-title"><span>${idx+1}</span><div><b>${esc(p.title||'پلن جدید')}</b><small>${p.price_currency==='USD'?'USD':'تومان'} · ${Number(p.duration_days||0)} روز · ${esc(p.delivery_type||'manual')}</small></div><button type="button" class="danger ghost" data-mc-remove-plan="${p.key}" data-group-key="${g.key}">حذف</button></div>
        <div class="form-grid mcw-grid compact">
        ...
        <div class="mcw-inline-setting">${mcSettingRow('فعال','این پلن در انتخاب‌های کاربر نمایش داده شود','data-mcpf="is_active"',Number(p.is_active),'●')}</div>
      </article>`).join('')||'<p class="muted mcw-no-plan">هنوز پلنی نیست.</p>'}</div></section>`).join('')}</div>`;
  }
