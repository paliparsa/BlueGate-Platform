...
function openAdminActionSheet(type,id){
  ...
  const body=`<div class="order-control-center">
    ...
    <section class="oac-section"><header><span>⚡</span><div><b>تحویل سرویس</b><small>تمام روش‌های قبلی حفظ شده‌اند</small></div></header><div class="oac-action-grid"><button data-admin-service="${o.id}"><span>🔗</span><b>ثبت لینک سرویس</b></button><button data-admin-deliver="${o.id}"><span>▤</span><b>تحویل متنی</b></button>${o.receipt_file_id?`<button data-view-receipt="${o.id}"><span>🧾</span><b>دیدن رسید</b></button>`:''}</div></section>
...
function setAdminTab(tab){if(['products','categories','variants'].includes(tab))tab='catalog';if(!['dashboard','catalog','orders','more','notifications','inventory','coupons','settings','activity','roles','backups'].includes(tab))tab='dashboard';currentAdminTab=tab;saveAppLastState();renderAdmin();miniScrollTop()}
