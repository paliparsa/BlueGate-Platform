...
function openMiniSupport(){
  ...
  prepareMiniLayer('previewSheet');const pv=$('previewSheet');if(!pv)return;
  const tickets=state.support_tickets||[];
  pv.innerHTML=`<div class="preview-sheet-inner"><div class="preview-sheet-handle" data-close-preview></div><div class="hub-card-head"><div><b>🎫 پشتیبانی BlueGate</b><small>${nf(tickets.length)} تیکت</small></div><button data-support-ticket-new>+ تیکت جدید</button></div><div class="bg-admin-list">${tickets.map(t=>`<button class="account-menu-row" data-support-ticket="${t.id}"><span class="account-menu-icon">#${t.id}</span><div><b>${esc(t.subject)}</b><small>${esc(t.status)} · ${esc(t.category||'other')}</small></div><em>‹</em></button>`).join('')||'<div class="u-empty"><h3>تیکتی نداری</h3><p>اگر مشکلی داری یک تیکت جدید بساز.</p></div>'}</div></div>`;pv.classList.add('open');pv.querySelectorAll('[data-close-preview]').forEach(x=>x.addEventListener('click',closePreviewSheet));
}
...
function renderAdminAdvanced(){
  const A=adminState.advanced||{},pa=A.provider_analytics||[];
  return `<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🚀</span><div><h3>امکانات پیشرفته</h3><p class="muted">تست رایگان، پشتیبانی، هزینه و ظرفیت پنل‌ها</p></div></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(A.trials_active||0)}</b><span>تست فعال</span></div><div class="dq-stat"><b>${nf(A.tickets_open||0)}</b><span>تیکت باز</span></div><div class="dq-stat"><b>${nf(pa.length)}</b><span>پنل</span></div></div></article>${pa.map(p=>`<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">📈</span><div><h3>${esc(p.name||'پنل')}</h3><p class="muted">${p.driver==='xui'?'3x-ui':esc(p.driver||'')} · ${nf(p.active_services||0)} سرویس · ${(Number(p.traffic_used_bytes||0)/1073741824).toFixed(1)} GB مصرف</p></div><span class="chip-mini ${p.maintenance_mode?'chip-off':'chip-active'}">${p.maintenance_mode?'تعمیرات':'فعال'}</span></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${esc(p.monthly_cost||0)}</b><span>هزینه ${esc(p.cost_currency||'USD')}</span></div><div class="dq-stat"><b>${nf(p.revenue_toman||0)}</b><span>درآمد</span></div><div class="dq-stat"><b>${nf(p.soft_capacity_percent||90)}%</b><span>ظرفیت نرم</span></div></div></article>`).join('')||'<article class="admin-card"><p class="muted">هنوز داده تحلیلی برای پنل‌ها وجود ندارد.</p></article>'}`;
}
