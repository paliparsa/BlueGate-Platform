...
/* ===== v3.0.9: parent/child layer stack ===== */
const MINI_LAYER_IDS=['appSheet','miniPurchaseOverlay','miniCreditTopupSheet','shareSheet','previewSheet','qrSheet','cartSheet','custDrawer','palettePopup','cmdPalette','onboarding','lightbox','authModal'];
const MINI_LAYER_KIND={
  appSheet:'primary',miniPurchaseOverlay:'primary',miniCreditTopupSheet:'primary',cartSheet:'primary',custDrawer:'primary',onboarding:'primary',authModal:'primary',
  shareSheet:'child',previewSheet:'child',qrSheet:'child',palettePopup:'child',cmdPalette:'child',lightbox:'child'
};
let miniLayerStack=[];
function miniLayerIsOpen(id){const el=$(id);if(!el)return false;if(id==='cmdPalette')return !el.hidden&&el.classList.contains('open');if(id==='authModal')return el.open||el.classList.contains('open');return el.classList.contains('open')}
function closeMiniLayerById(id){
  if(id==='appSheet'){window.BlueGateUI?.closeSheet?.();return true}
  if(id==='miniPurchaseOverlay'){window.BlueGatePurchase?.close?.();return true}
  if(id==='miniCreditTopupSheet'){closeMiniCreditTopup();return true}
  if(id==='shareSheet'){closeShareSheet();return true}
  if(id==='previewSheet'){closePreviewSheet();return true}
  if(id==='qrSheet'){closeQrSheet();return true}
  if(id==='cartSheet'){closeCartSheet();return true}
  if(id==='custDrawer'){closeCustomer360();return true}
  if(id==='palettePopup'){closePalettePopup();return true}
  if(id==='cmdPalette'){closeCommandPalette();return true}
  if(id==='onboarding'){$('onboarding')?.classList.remove('open');return true}
  if(id==='lightbox'){closeLightbox();return true}
  if(id==='authModal'){closeAuthModal();return true}
  return false;
}
function pruneMiniLayerStack(){miniLayerStack=miniLayerStack.filter((id,i,a)=>a.indexOf(id)===i&&miniLayerIsOpen(id));return miniLayerStack}
function registerMiniLayer(id){miniLayerStack=pruneMiniLayerStack().filter(x=>x!==id);miniLayerStack.push(id)}
function prepareMiniLayer(id){
  const kind=MINI_LAYER_KIND[id]||'child';
  pruneMiniLayerStack();
  if(kind==='primary'){
    // Primary pages replace other primary pages and clear orphan child overlays.
    [...miniLayerStack].reverse().forEach(openId=>{if(openId!==id&&miniLayerIsOpen(openId))closeMiniLayerById(openId)});
    miniLayerStack=[];
  }else if(miniLayerIsOpen(id)){
    // Reopening the same child replaces only itself, never its parent.
    closeMiniLayerById(id);
  }
  registerMiniLayer(id);
}
function topMiniLayer(){pruneMiniLayerStack();for(let i=miniLayerStack.length-1;i>=0;i--){if(miniLayerIsOpen(miniLayerStack[i]))return miniLayerStack[i]}for(let i=MINI_LAYER_IDS.length-1;i>=0;i--){if(miniLayerIsOpen(MINI_LAYER_IDS[i]))return MINI_LAYER_IDS[i]}return null}
function closeTopMiniLayer(){const id=topMiniLayer();if(!id)return false;const ok=closeMiniLayerById(id);miniLayerStack=miniLayerStack.filter(x=>x!==id);return ok}
document.addEventListener('bluegate:before-layer-open',e=>{const id=e.detail?.id;if(id)prepareMiniLayer(id)});
function syncTelegramBackButton(){try{const bb=tg?.BackButton;if(!bb)return;if(topMiniLayer()||currentOrderId||currentServiceOrderId||currentTab!=='shop')bb.show();else bb.hide()}catch(_){}}
setInterval(syncTelegramBackButton,700);
try{tg?.BackButton?.onClick?.(()=>{if(closeTopMiniLayer()){syncTelegramBackButton();return}if(currentServiceOrderId){currentServiceOrderId=null;currentTab='home';renderUser();miniScrollTop();return}if(currentOrderId){currentOrderId=null;currentTab='orders';renderUser();return}if(currentTab!=='shop'){currentTab='shop';renderUser();miniScrollTop();return}})}catch(_){}
...
function restoreAppLastState(){
  ...
  try {
    ...
    if (savedAdmin.currentAdminTab) currentAdminTab = ['dashboard','catalog','orders','more','notifications','inventory','coupons','settings','activity','roles','backups'].includes(savedAdmin.currentAdminTab) ? savedAdmin.currentAdminTab : 'dashboard';
...
function userPhotoUrl(u={}){return u.avatar_url || u.photo_url || tgUser().photo_url || ''}
...
/* Lightbox */
function openLightbox(url,caption=''){prepareMiniLayer('lightbox');const lb=$('lightbox');if(!lb)return;lb.innerHTML=`<div class="lightbox-backdrop"></div><img src="${esc(url)}" alt="${esc(caption)}"><button class="lightbox-close">✕</button>${caption?`<p class="lightbox-caption">${esc(caption)}</p>`:''}`;lb.classList.add('open');lb.querySelector('.lightbox-backdrop')?.addEventListener('click',()=>closeLightbox());lb.querySelector('.lightbox-close')?.addEventListener('click',()=>closeLightbox())}
...
async function loadCreditTopupReceipt(topupId){try{haptic('light');showStatus('در حال دریافت رسید شارژ...');const r=await api('get_credit_topup_receipt_url',{topup_id:Number(topupId)});if(r.url){openLightbox(r.url,`رسید افزایش اعتبار #${nf(topupId)}`);showStatus('')}else{showStatus('رسید قابل دریافت نبود','error')}}catch(e){showStatus(e.message||'خطا در دریافت رسید شارژ','error')}}
...
function openCartSheet(){prepareMiniLayer('cartSheet');const s=$('cartSheet');if(!s)return;s.innerHTML=cartSheetHtml();s.classList.add('open');s.style.display='flex';haptic('light');bindCartEvents(s)}
...
function openAdminActionSheet(type,id){
  ...
  const statusLabel=esc(o.status_fa||status||'بدون وضعیت');
  const statusClass=status==='rejected'||status==='canceled'?'danger':status==='delivered'?'success':'info';
  const primaryActions=[];
  if(o.receipt_file_id) primaryActions.push(`<button class="oac-primary" data-view-receipt="${o.id}"><span>🧾</span><b>رسید</b><small>مشاهده پرداخت</small></button>`);
  if(!['payment_confirmed','preparing','delivered'].includes(status)) primaryActions.push(`<button class="oac-primary success" data-admin-status="${o.id}:payment_confirmed"><span>✓</span><b>تأیید پرداخت</b><small>مرحله بعد</small></button>`);
  if(status!=='delivered') primaryActions.push(`<button class="oac-primary" data-admin-deliver="${o.id}"><span>↗</span><b>تحویل</b><small>ثبت متن تحویل</small></button>`);
  const body=`<div class="order-control-center">
    <div class="oac-summary"><div><span class="oac-order-id">#${nf(o.id)}</span><b>${esc(o.display_name||'سفارش')}</b><small>${o.username?'@'+esc(o.username):'کاربر سفارش'}</small></div><em class="oac-status ${statusClass}">${statusLabel}</em></div>
    ${primaryActions.length?`<div class="oac-primary-grid">${primaryActions.slice(0,3).join('')}</div>`:''}
    <section class="oac-section"><header><span>👤</span><div><b>کاربر</b><small>پروفایل و ارتباط</small></div></header><div class="oac-action-grid">${o.user_id?`<button data-customer360="${o.user_id}"><span>◎</span><b>پروفایل کاربر</b></button>`:''}${o.username?`<button data-chat-user="${esc(o.username)}"><span>✉</span><b>ارسال پیام</b></button>`:''}<button data-admin-order-note="${o.id}"><span>✎</span><b>یادداشت داخلی</b></button></div></section>
    <section class="oac-section"><header><span>◫</span><div><b>وضعیت سفارش</b><small>مرحله پردازش را انتخاب کن</small></div></header><div class="oac-status-grid"><button class="${status==='reviewing'?'active':''}" data-admin-status="${o.id}:reviewing"><span>1</span><b>در بررسی</b></button><button class="${status==='payment_confirmed'?'active':''}" data-admin-status="${o.id}:payment_confirmed"><span>2</span><b>تأیید پرداخت</b></button><button class="${status==='preparing'?'active':''}" data-admin-status="${o.id}:preparing"><span>3</span><b>آماده‌سازی</b></button></div></section>
    <section class="oac-section"><header><span>⚡</span><div><b>تحویل سرویس</b><small>ساخت خودکار از پنل یا تحویل دستی</small></div></header><div class="oac-action-grid">${o.catalog_plan_id&&!o.service_action&&!['rejected','canceled','refunded'].includes(status)?`<button class="success" data-admin-provision="${o.id}"><span>⚡</span><b>ساخت و ارسال از پنل</b></button>`:''}<button data-admin-service="${o.id}"><span>🔗</span><b>ثبت دستی لینک ساب</b></button><button data-admin-deliver="${o.id}"><span>▤</span><b>تحویل متنی</b></button>${o.receipt_file_id?`<button data-view-receipt="${o.id}"><span>🧾</span><b>دیدن رسید</b></button>`:''}</div></section>
    <details class="oac-danger"><summary><span>⚠</span><div><b>عملیات بیشتر</b><small>رد، آرشیو و حذف</small></div><i>⌄</i></summary><div><button class="danger" data-admin-status="${o.id}:rejected">رد سفارش</button><button class="danger soft" data-admin-archive-order="${o.id}">آرشیو سفارش</button>${cleanupStatuses.includes(o.status)?`<button class="danger hard" data-admin-delete-order="${o.id}">حذف کامل</button>`:''}</div></details>
  </div>`;
  BlueGateUI.openSheet({type:'action',eyebrow:`ORDER CONTROL · #${nf(o.id)}`,title:o.display_name||'سفارش',subtitle:statusLabel,body});
}
...
/* Onboarding (U10) */
const BLUEGATE_WELCOME_VERSION=1;
function shouldShowOnboarding(){if(state?.is_guest||!state?.user)return false;try{if(localStorage.getItem('bluegate_welcome_seen_v'+BLUEGATE_WELCOME_VERSION)==='1')return false}catch(_){}return Number(state.user.welcome_version_seen||0)<BLUEGATE_WELCOME_VERSION}
function showOnboarding(){prepareMiniLayer('onboarding');if(!shouldShowOnboarding())return;const o=$('onboarding');if(!o)return;const slides=[{emoji:'🛍️',title:'همه سرویس‌ها، یکجا',text:'خرید، تمدید و مدیریت سرویس‌های BlueGate را بدون خروج از Mini App انجام بده.'},{emoji:'⚡',title:'دسترسی سریع به BluePing',text:'سرویس VPN را باز کن، لینک Subscription را کپی کن و تمدید را مستقیم انجام بده.'},{emoji:'🔔',title:'هیچ اطلاعیه‌ای را از دست نده',text:'تحویل سفارش، تغییرات سرویس و پیام‌های مهم BlueGate داخل مرکز اعلان‌ها نمایش داده می‌شوند.'},{emoji:'👤',title:'حساب و اعتبارت همیشه همراهته',text:'اطلاعات حساب، اعتبار، سفارش‌ها و راه‌های بازیابی دسترسی را از بخش حساب مدیریت کن.'}];let idx=0;o.innerHTML=`<div class="onb-inner welcome-v1"><button class="welcome-close" id="welcomeClose" type="button" aria-label="بستن">×</button><div class="welcome-brand"><span>BG</span><div><b>BlueGate</b><small>WELCOME</small></div></div><div class="onb-slides">${slides.map((x,i)=>`<div class="onb-slide ${i===0?'active':''}" data-onb-slide="${i}"><div class="onb-emoji">${x.emoji}</div><h2>${x.title}</h2><p>${x.text}</p></div>`).join('')}</div><div class="onb-dots">${slides.map((_,i)=>`<span class="onb-dot ${i===0?'active':''}" data-onb-dot="${i}"></span>`).join('')}</div><label class="welcome-dontshow"><input type="checkbox" id="welcomeDontShow"><span></span><b>دیگر نمایش نده</b></label><div class="onb-actions"><button class="ghost" id="onbSkip">فعلاً رد کن</button><button class="primary" id="onbNext">بعدی</button></div></div>`;o.classList.add('open');const next=$('onbNext');next?.addEventListener('click',()=>{idx++;if(idx>=slides.length){finishOnboarding();return}updateOnbSlide(idx,slides.length)});$('onbSkip')?.addEventListener('click',finishOnboarding);$('welcomeClose')?.addEventListener('click',()=>o.classList.remove('open'));o.querySelectorAll('[data-onb-dot]').forEach(d=>d.addEventListener('click',()=>{idx=Number(d.dataset.onbDot);updateOnbSlide(idx,slides.length)}))}
...
async function finishOnboarding(){const remember=!!$('welcomeDontShow')?.checked;$('onboarding')?.classList.remove('open');if(!remember)return;try{const r=await api('welcome_seen',{version:BLUEGATE_WELCOME_VERSION});if(state?.user)state.user.welcome_version_seen=Number(r.welcome_version_seen||BLUEGATE_WELCOME_VERSION)}catch(_){try{localStorage.setItem('bluegate_welcome_seen_v'+BLUEGATE_WELCOME_VERSION,'1')}catch(__){}}}
/* Recently viewed (U12) */
...
function openCommandPalette(){prepareMiniLayer('cmdPalette');
  const cp=$('cmdPalette');
...
function productCardPriceLine(p){
  ...
  if(discount?.kind==='variant'&&discount.variantId){
    ...
    if(sale>0) return `<span>از</span><strong>${fmt(sale)}</strong>`;
  }
  const directDiscount=Number(p?.discount_percent||0)>0&&Number(p?.price||0)>0?Number(p.price):0;
  if(directDiscount>0) return `<span>از</span><strong>${fmt(directDiscount)}</strong>`;
  const min=productMinPriceNumber(p);
  if(min>0) return `<span>از</span><strong>${fmt(min)}</strong>`;
...
function miniScrollTop(){requestAnimationFrame(()=>{try{window.scrollTo({top:0,left:0,behavior:'auto'});if(document.scrollingElement)document.scrollingElement.scrollTop=0;document.body.scrollTop=0;document.documentElement.scrollTop=0}catch(_){}})}
function setTab(tab){
...
function setAdminTab(tab){if(['products','categories','variants'].includes(tab))tab='catalog';if(!['dashboard','catalog','orders','more','infrastructure','advanced','notifications','inventory','coupons','settings','activity','roles','backups'].includes(tab))tab='dashboard';currentAdminTab=tab;saveAppLastState();renderAdmin();miniScrollTop()}
function scheduleAvatarSync(){const u=state?.user||{};if(state?.is_guest||u.avatar_source==='custom'||!u.telegram_connected)return;const last=u.avatar_synced_at?Date.parse(String(u.avatar_synced_at).replace(' ','T')):0;if(last&&Date.now()-last<86400000)return;const run=async()=>{try{const fresh=await api('sync_my_telegram_avatar');if(fresh?.user){state=fresh;updateMiniHeader();if(currentTab==='home')renderHome()}}catch(_){}};if('requestIdleCallback' in window)requestIdleCallback(run,{timeout:2500});else setTimeout(run,900)}
function render(data){
  ...
  renderUser();checkAndCelebrate();handleDeepLink();scheduleAvatarSync();
}
...
function notificationIcon(n){return n.type==='delivered'?'✅':n.type==='wallet'?'💳':n.type==='important'?'⚠️':n.type==='promo'?'🎁':n.type==='service'?'⚡':n.type==='security'?'🛡️':'🔔'}
function runMiniNotificationAction(n){const a=String(n.action_type||'');const v=String(n.action_value||'');BlueGateUI.closeSheet();if(n.order_id||a==='orders'){currentTab='orders';currentOrderId=Number(n.order_id||v||0)||null;renderUser();miniScrollTop();return}if(a==='store'){currentTab='shop';renderUser();return}if(a==='wallet'){currentTab='wallet';renderUser();miniScrollTop();return}if(a==='account'||a==='profile'){currentTab='home';renderUser();return}if(a==='service'&&v){currentServiceOrderId=Number(v)||null;currentTab='home';renderUser();return}if(a==='url'&&/^https:\/\//i.test(v)){if(TG?.openLink)TG.openLink(v);else window.open(v,'_blank','noopener')}}
function openNotifications(){const list=state?.notifications||[];BlueGateUI.openSheet({type:'action',eyebrow:'BLUEGATE',title:'اعلان‌ها',subtitle:Number(state?.notification_unread||0)?`${nf(state.notification_unread)} اعلان خوانده‌نشده`:'همه اعلان‌ها دیده شده‌اند',body:`<div class="mini-notification-list">${list.length?list.map(n=>`<button class="mini-notification-row ${n.is_read?'':'unread'}" data-notification-id="${n.id}"><span>${notificationIcon(n)}</span><div><b>${esc(n.title)}</b><small>${esc(n.body||'')}</small><em>${esc(String(n.created_at||'').slice(0,16))}${n.action_type?' · مشاهده ←':''}</em></div></button>`).join(''):'<div class="u-empty"><div class="u-empty-icon">🔔</div><h3>اعلانی نداری</h3><p>تحویل سفارش‌ها و پیام‌های BlueGate اینجا نمایش داده می‌شود.</p></div>'}</div>`,onOpen:(host)=>{host.querySelectorAll('[data-notification-id]').forEach(x=>x.addEventListener('click',()=>{const n=list.find(y=>Number(y.id)===Number(x.dataset.notificationId));if(n&&(n.order_id||n.action_type))runMiniNotificationAction(n)}));}});if(Number(state?.notification_unread||0)>0){state.notification_unread=0;(state.notifications||[]).forEach(n=>n.is_read=1);updateNotificationBadge();api('notifications_read').catch(()=>{})}}
...
function myServiceById(id){
  const n=Number(id||0);
  if(n<0){const o=(state.orders||[]).find(x=>Number(x.id)===Math.abs(n));return o?{...o,id:n,order_id:Number(o.id),_manual_order:true,subscription_url:o.service_url||o.delivery_url||null,status:o.status==='delivered'?'active':o.status}:null;}
  return (state.services||[]).find(x=>Number(x.id)===n)||(state.orders||[]).find(x=>Number(x.id)===n)||null;
}
function serviceHubItems(){
  const managed=[...(state.services||[])];const represented=new Set(managed.map(x=>Number(x.order_id||0)).filter(Boolean));
  const manual=(state.orders||[]).filter(o=>o.status==='delivered'&&isVpnService(o)&&String(o.service_url||o.delivery_url||'').trim()&&!represented.has(Number(o.id))).map(o=>({...o,id:-Number(o.id),order_id:Number(o.id),_manual_order:true,subscription_url:o.service_url||o.delivery_url,status:'active',display_name:o.display_name||o.product_name||'سرویس VPN'}));
...
function bytesFa(v){const n=Number(v||0);if(!n)return '0 GB';return (n/1073741824).toLocaleString('fa-IR',{maximumFractionDigits:1})+' GB'}
function serviceForOrder(o){
  if(!o)return null;
  const managed=(state.services||[]).find(x=>Number(x.order_id||0)===Number(o.id)||Number(x.id||0)===Number(o.user_service_id||0));
  if(managed)return managed;
  if(!isVpnService(o))return null;
  return {...o,id:-Number(o.id),order_id:Number(o.id),_manual_order:true,subscription_url:o.service_url||o.delivery_url||null,status:o.status==='delivered'?'active':o.status,display_name:o.display_name||o.product_name||'سرویس VPN'};
}
function vpnStatusFa(s){s=String(s||'active').toLowerCase();return s==='active'?'فعال':s==='preparing'||s==='provisioning'?'در حال ساخت':s==='payment_confirmed'?'پرداخت تایید شده':s==='pending_payment'||s==='pending'?'در انتظار پرداخت':s==='suspended'?'متوقف':s==='expired'?'منقضی':s==='degraded'?'نیازمند بررسی':s==='provisioning_failed'?'خطای ساخت':s}
function vpnDaysLeft(v){if(!v)return null;const t=Date.parse(String(v).replace(' ','T'));if(!Number.isFinite(t))return null;return Math.max(0,Math.ceil((t-Date.now())/86400000))}
function vpnFlagToCode(flag){try{return [...String(flag)].map(c=>String.fromCharCode(c.codePointAt(0)-0x1F1E6+65)).join('')}catch(_){return''}}
const VPN_LOCATION_FA={TR:'ترکیه',DE:'آلمان',SE:'سوئد',GB:'انگلستان',NL:'هلند',FR:'فرانسه',FI:'فنلاند',US:'آمریکا',CA:'کانادا',AE:'امارات',IR:'ایران',CH:'سوئیس',AT:'اتریش',IT:'ایتالیا',ES:'اسپانیا',PL:'لهستان',RO:'رومانی',CZ:'چک',NO:'نروژ',DK:'دانمارک',RU:'روسیه',JP:'ژاپن',SG:'سنگاپور',IN:'هند',HK:'هنگ‌کنگ',AU:'استرالیا',BR:'برزیل',ZA:'آفریقای جنوبی',IE:'ایرلند',BE:'بلژیک',PT:'پرتغال',UA:'اوکراین',GR:'یونان',BG:'بلغارستان'};
const VPN_LOCATION_WORDS=[['turkey','TR','🇹🇷'],['türkiye','TR','🇹🇷'],['germany','DE','🇩🇪'],['deutschland','DE','🇩🇪'],['sweden','SE','🇸🇪'],['stockholm','SE','🇸🇪'],['uk','GB','🇬🇧'],['england','GB','🇬🇧'],['london','GB','🇬🇧'],['netherlands','NL','🇳🇱'],['holland','NL','🇳🇱'],['france','FR','🇫🇷'],['finland','FI','🇫🇮'],['usa','US','🇺🇸'],['united states','US','🇺🇸'],['canada','CA','🇨🇦'],['emirates','AE','🇦🇪'],['dubai','AE','🇦🇪'],['iran','IR','🇮🇷'],['switzerland','CH','🇨🇭'],['italy','IT','🇮🇹'],['spain','ES','🇪🇸'],['poland','PL','🇵🇱'],['romania','RO','🇷🇴'],['norway','NO','🇳🇴'],['denmark','DK','🇩🇰'],['russia','RU','🇷🇺'],['japan','JP','🇯🇵'],['singapore','SG','🇸🇬'],['india','IN','🇮🇳'],['australia','AU','🇦🇺']];
function vpnTargetNames(o){const out=[];(o.instances||[]).forEach(i=>{(i.targets||[]).forEach(t=>{const n=String(t.remote_target_name||'').trim();if(n)out.push(n)})});return out}
function vpnSmartLocations(o){const found=new Map();for(const name of vpnTargetNames(o)){const flags=name.match(/[\u{1F1E6}-\u{1F1FF}]{2}/gu)||[];for(const flag of flags){const code=vpnFlagToCode(flag);if(code&&!found.has(code))found.set(code,{code,flag,name:VPN_LOCATION_FA[code]||name.replace(/[\u{1F1E6}-\u{1F1FF}]{2}/gu,'').trim()||code})}if(flags.length)continue;const low=name.toLowerCase();for(const [word,code,flag] of VPN_LOCATION_WORDS){if(low.includes(word)&&!found.has(code)){found.set(code,{code,flag,name:VPN_LOCATION_FA[code]||name});break}}}return [...found.values()]}
function vpnServiceDetailHtml(o,{context='service',order=null}={}){
  const hasUrl=!!String(o.subscription_url||o.service_url||o.delivery_url||'').trim();
  const url=String(o.subscription_url||o.service_url||o.delivery_url||'');
  ...
  const limit=Number(o.traffic_limit_bytes||0),used=Number(o.traffic_used_bytes||0),remaining=Math.max(0,limit-used),pct=limit?Math.min(100,Math.round(used/limit*100)):0;
  const status=String(o.status||'active');const statusFa=vpnStatusFa(status);const add=o.addons||{};const manual=!!o._manual_order;const clientName=(o.instances||[])[0]?.external_username||o.client_username||'';
  const days=vpnDaysLeft(o.expires_at);const smartLocations=vpnSmartLocations(o);const locations=smartLocations.length||((o.instances||[]).length?1:0);
  const back=context==='order'?`<button class="vpn-sub-back" data-vpn-detail-back="orders">‹ <span>سفارش‌ها</span></button>`:`<button class="vpn-sub-back" data-vpn-detail-back="services">‹ <span>سرویس‌های من</span></button>`;
  const orderId=Number(order?.id||o.order_id||0);
  const waitingPay=order&&['pending_payment','pending','rejected'].includes(String(order.status||''));
  return `<section class="vpn-sub-page">
    <div class="vpn-sub-topbar">${back}<div class="vpn-sub-topmeta">${orderId?`<span>سفارش #${nf(orderId)}</span>`:''}<b class="vpn-sub-status ${status==='active'?'ok':status==='expired'||status==='provisioning_failed'?'bad':'warn'}"><i></i>${esc(statusFa)}</b></div></div>
    <article class="vpn-sub-identity">
      <div class="vpn-sub-copy"><div><div class="vpn-sub-micro"><i></i> BLUEGATE MANAGED VPN</div><h1>${title}</h1><p>${hasUrl?'سرویس آماده استفاده است. لینک اشتراک را مستقیم در کلاینت VPN باز کن یا کپی کن.':'سرویس شما ثبت شده و پس از ساخت Client، لینک اشتراک همین‌جا فعال می‌شود.'}</p></div><div class="vpn-sub-pills"><span class="${status==='active'?'ok':'warn'}"><i></i>${esc(statusFa)}</span>${locations?`<span>◉ ${nf(locations)} مسیر فعال</span>`:''}${clientName?`<span>Client: <b>${esc(clientName)}</b></span>`:''}</div></div>
      <div class="vpn-sub-orb-wrap"><div class="vpn-sub-orb" style="--p:${pct}"><div><strong>${limit?pct.toLocaleString('fa-IR')+'٪':'∞'}</strong><span>${limit?'مصرف شده':'حجم نامحدود'}</span></div></div><small>${limit?`${bytesFa(remaining)} باقی‌مانده`:'بدون سقف ترافیک'}</small></div>
    </article>
    <div class="vpn-sub-overview">
      <article class="vpn-sub-panel"><header><div><small>TRAFFIC</small><h3>مصرف سرویس</h3></div><b>${limit?bytesFa(used):'نامحدود'}</b></header>${limit?`<div class="vpn-sub-progress"><i style="width:${pct}%"></i></div>`:''}<div class="vpn-sub-duo"><div><span>مصرف‌شده</span><b>${limit?bytesFa(used):'—'}</b></div><div><span>باقی‌مانده</span><b>${limit?bytesFa(remaining):'نامحدود'}</b></div></div></article>
      <article class="vpn-sub-panel"><header><div><small>EXPIRY</small><h3>اعتبار سرویس</h3></div><b>${days===null?'∞':nf(days)+' روز'}</b></header><div class="vpn-sub-expiry"><strong>${days===null?'بدون انقضا':nf(days)}</strong><span>${days===null?'سرویس بدون تاریخ پایان':'روز باقی‌مانده'}</span></div><div class="vpn-sub-date">${expiry}</div></article>
    </div>
    <div class="vpn-sub-metrics"><div><i>⚡</i><span>وضعیت</span><b>${esc(statusFa)}</b></div><div><i>◎</i><span>لوکیشن‌ها</span><b>${nf(locations||1)}</b></div><div><i>↻</i><span>آخرین Sync</span><b>${esc(o.last_sync_at?String(o.last_sync_at).slice(0,16):'—')}</b></div><div><i>◈</i><span>نوع ساب</span><b>${o.subscription_mode==='bluegate_aggregate'?'Aggregate':'Native'}</b></div></div>
    <article class="vpn-subscription-card ${hasUrl?'ready':'pending'}"><header><div><small>SUBSCRIPTION</small><h3>${hasUrl?'لینک اشتراک شما':'در حال آماده‌سازی لینک'}</h3></div><span>${hasUrl?'آماده ✓':'…'}</span></header>${hasUrl?`<code>${esc(url)}</code><div class="vpn-sub-actions"><button class="primary" data-service-view="${o.id}" data-service-url="${esc(url)}" data-service-title="Subscription">↗ باز کردن ساب</button><button data-service-copy-link="${o.id}" data-service-url="${esc(url)}">⧉ کپی لینک</button><button data-service-qr="${o.id}" data-service-url="${esc(url)}">▦ QR</button></div>`:`<div class="vpn-sub-pending"><span>⚙️</span><div><b>Client هنوز آماده نشده</b><small>بعد از ساخت از پنل، لینک Subscription به‌صورت خودکار اینجا نمایش داده می‌شود.</small></div></div>`}</article>
    ${smartLocations.length?`<article class="vpn-sub-targets"><header><small>LOCATIONS</small><h3>لوکیشن‌های سرویس</h3></header><div>${smartLocations.map(l=>`<section><span class="vpn-target-proto vpn-target-flag">${esc(l.flag)}</span><div><b>${esc(l.name)}</b><small>${esc(l.code)} · فعال در Subscription</small></div><em>فعال</em></section>`).join('')}</div></article>`:''}
    <div class="vpn-sub-quick">${manual?`<button data-renew-order="${o.order_id}"><i>↻</i><b>تمدید</b><small>ساخت سفارش جدید</small></button>`:`<button data-service-renew="${o.id}"><i>↻</i><b>تمدید</b><small>همین Client</small></button><button data-service-sync="${o.id}"><i>⟳</i><b>بروزرسانی</b><small>مصرف و انقضا</small></button>`}${Number(add.traffic_price_per_gb||0)>0?`<button data-service-add-traffic="${o.id}"><i>＋</i><b>حجم اضافه</b><small>افزایش ترافیک</small></button>`:''}${Number(add.day_price||0)>0?`<button data-service-add-days="${o.id}"><i>＋</i><b>روز اضافه</b><small>افزایش اعتبار</small></button>`:''}${!manual?`<button data-service-history="${o.id}"><i>▥</i><b>نمودار مصرف</b><small>۳۰ روز اخیر</small></button><button data-service-events="${o.id}"><i>◷</i><b>رویدادها</b><small>تاریخچه سرویس</small></button>`:''}</div>
    ${waitingPay?`<article class="vpn-sub-payment"><h3>تکمیل پرداخت</h3>${paymentMethodsHtml(order)}</article>`:''}
  </section>`;
}
...
function renderHome(){
  ...
  const missingEmail=!String(u.email||'').trim(),missingPhone=!String(u.phone_number||'').trim();const securityBanner=!isGuest&&(missingEmail||missingPhone)?`<article class="account-security-alert"><span>⚠️</span><div><b>امنیت حسابت رو کامل کن</b><p>${missingEmail&&missingPhone?'برای افزایش امنیت حساب و بازیابی دسترسی در زمان اختلال یا قطعی اینترنت، ایمیل و شماره تماس خودت را اضافه کن.':missingEmail?'برای افزایش امنیت و بازیابی حساب، ایمیل خودت را اضافه کن.':'برای افزایش امنیت و بازیابی حساب، شماره تماس خودت را اضافه کن.'}</p></div><button id="completeMiniSecurity">تکمیل اطلاعات</button></article>`:'';
  const services=recent.length?recent.map(o=>`<div class="hub-service-row"><button class="hub-order-row" data-my-service="${o.id}"><div><b>${esc(o.display_name||o.product_name||'سرویس')}</b><small>${o.expires_at?'فعال تا '+esc(String(o.expires_at).slice(0,10)):'سرویس فعال'}</small></div><span>›</span></button><button class="mini-renew-inline" ${o._manual_order?`data-renew-order="${o.order_id}"`:`data-service-renew="${o.id}"`}>تمدید</button></div>`).join(''):`<div class="u-empty"><div class="u-empty-icon">◇</div><h3>هنوز سرویس فعالی نداری</h3><p>از فروشگاه اولین سرویس BlueGate خودت رو انتخاب کن.</p><button class="primary" data-tab-jump="shop">رفتن به فروشگاه</button></div>`;
  $('homePage').innerHTML=`${guestBanner}${securityBanner}
    ...
    ${state?.trial?.eligible?`<article class="account-hub-card"><div class="hub-card-head"><span>🧪</span><div><b>تست رایگان BlueGate</b><small>${nf(Math.round(Number(state.trial?.settings?.traffic_limit_bytes||0)/1073741824))} GB · ${nf(state.trial?.settings?.duration_minutes||120)} دقیقه</small></div><button id="miniTrialBtn">فعال‌سازی</button></div></article>`:''}
...
function openMiniProfileEditor(){
  ...
  const o=miniTopupShell('ویرایش اطلاعات حساب','اطلاعات پایه حسابت را به‌روزرسانی کن.',`<div class="mini-account-editor">
    <div class="mini-account-editor-summary mini-avatar-editor-summary">${userProfileAvatar(u,'mini-profile-edit-avatar')}<div><b>${esc(u.first_name||u.username||'کاربر BlueGate')}</b><small>${esc(u.username?'@'+u.username:'حساب BlueGate')}</small></div><label class="mini-avatar-upload-btn">تغییر عکس<input id="miniAvatarFile" type="file" accept="image/jpeg,image/png,image/webp" hidden></label></div><div class="mini-avatar-actions"><button type="button" class="ghost" id="miniAvatarTelegram">↻ عکس تلگرام</button><button type="button" class="ghost" id="miniAvatarRemove">استفاده از تلگرام</button></div>
  ...
  o.querySelector('#miniAvatarFile')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;if(file.size>4*1024*1024){showStatus('حجم عکس باید کمتر از ۴MB باشد','error');return}try{const data=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(file)});state=await api('update_my_avatar',{image_b64:data});closeMiniCreditTopup();renderUser();showStatus('عکس پروفایل ذخیره شد','success');setTimeout(openMiniProfileEditor,80)}catch(err){showStatus(err.message||'آپلود عکس انجام نشد','error')}});
  o.querySelector('#miniAvatarTelegram')?.addEventListener('click',async()=>{try{state=await api('sync_my_telegram_avatar');closeMiniCreditTopup();renderUser();showStatus('عکس تلگرام بروزرسانی شد','success');setTimeout(openMiniProfileEditor,80)}catch(err){showStatus(err.message||'دریافت عکس تلگرام ممکن نشد','error')}});
  o.querySelector('#miniAvatarRemove')?.addEventListener('click',async()=>{try{state=await api('reset_my_avatar');closeMiniCreditTopup();renderUser();showStatus('عکس تلگرام جایگزین شد','success');setTimeout(openMiniProfileEditor,80)}catch(err){showStatus(err.message||'تغییر عکس انجام نشد','error')}});
...
function catalogGroupPriceLabel(g){const prices=(g?.plans||[]).map(x=>Number(x.price||0)).filter(x=>x>0);return prices.length?`از ${fmt(Math.min(...prices))}`:''}
...
function openShareSheet(pid){prepareMiniLayer('shareSheet');
...
function openMiniSupport(){
  if(state?.is_guest){openAuthModal();return}
  const url=miniSupportUrl();
  if(TG?.openTelegramLink)TG.openTelegramLink(url);else if(TG?.openLink)TG.openLink(url);else window.open(url,'_blank','noopener');
}
function newMiniSupportTicket(){openDialog('تیکت پشتیبانی','موضوع تیکت را وارد کن.','مثلاً مشکل سرویس',async(subject)=>{if(!subject)return;openDialog('پیام تیکت','مشکل یا درخواستت را کامل بنویس.','متن پیام',async(message)=>{if(!message)return;try{const r=await api('support_ticket_create',{subject,message,category:'other'});state.support_tickets=r.tickets||[];showStatus('تیکت پشتیبانی ثبت شد','success');openMiniSupport()}catch(e){showStatus(e.message||'ثبت تیکت انجام نشد','error')}})})}
async function openMiniSupportTicket(id){try{const r=await api('support_ticket_messages',{ticket_id:Number(id)});prepareMiniLayer('previewSheet');const pv=$('previewSheet');const t=(state.support_tickets||[]).find(x=>Number(x.id)===Number(id))||{};pv.innerHTML=`<div class="preview-sheet-inner"><div class="preview-sheet-handle" data-close-preview></div><div class="hub-card-head"><div><b>#${id} ${esc(t.subject||'تیکت')}</b><small>${esc(t.status||'')}</small></div></div><div class="bg-admin-list">${(r.messages||[]).map(m=>`<article class="my-service-info-card"><div><span>${m.sender_type==='admin'?'پشتیبانی':'شما'}</span><small>${esc(m.created_at||'')}</small></div><p>${esc(m.body||'')}</p></article>`).join('')||'<div class="u-empty">پیامی نیست.</div>'}</div><button class="primary" data-support-ticket-reply="${id}">پاسخ به تیکت</button><button class="ghost" id="miniSupportBack">بازگشت</button></div>`;pv.classList.add('open');pv.querySelectorAll('[data-close-preview]').forEach(x=>x.addEventListener('click',closePreviewSheet));$('miniSupportBack')?.addEventListener('click',openMiniSupport)}catch(e){showStatus(e.message||'باز کردن تیکت ممکن نشد','error')}}
...
function renderAdmin(){
  ...
  const renderers={
    ...
    infrastructure:()=>renderAdminInfrastructure(),
    advanced:()=>renderAdminAdvanced()
  };
...
function renderAdminMore(){return `<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">•••</span><div><h3>ابزارهای بیشتر</h3><p class="muted">بخش‌های مدیریتی تکمیلی در یک جای مشخص.</p></div></div><div class="admin-more-grid"><button class="admin-more-link" data-admin-tab="infrastructure"><span>🧩</span><div><b>زیرساخت سرویس</b><small>پنل‌ها، اتصال پلن، سلامت و صف</small></div></button><button class="admin-more-link" data-admin-tab="advanced"><span>🚀</span><div><b>امکانات پیشرفته</b><small>تست رایگان، هزینه، ظرفیت و اتوماسیون</small></div></button><button class="admin-more-link" data-admin-tab="notifications"><span>🔔</span><div><b>اعلان‌ها</b><small>ارسال پیام داخل وب و Mini App</small></div></button><button class="admin-more-link" data-admin-tab="inventory"><span>□</span><div><b>انبار</b><small>آیتم‌های تحویل و موجودی</small></div></button><button class="admin-more-link" data-admin-tab="coupons"><span>%</span><div><b>تخفیف‌ها</b><small>کد تخفیف و کمپین‌ها</small></div></button><button class="admin-more-link" data-admin-tab="settings"><span>⚙</span><div><b>تنظیمات</b><small>پرداخت، ظاهر و فروشگاه</small></div></button><button class="admin-more-link" data-admin-tab="activity"><span>≡</span><div><b>فعالیت</b><small>لاگ تغییرات مدیریتی</small></div></button><button class="admin-more-link" data-admin-tab="roles"><span>◎</span><div><b>نقش‌ها</b><small>دسترسی مدیران</small></div></button><button class="admin-more-link" data-admin-tab="backups"><span>⇩</span><div><b>بکاپ</b><small>نگهداری و بازیابی سیستم</small></div></button></div></article><article class="admin-card admin-maintenance-card"><div class="admin-card-head"><span class="admin-card-icon">⌁</span><div><h3>عملیات سریع</h3><p class="muted">ابزارهای عملیاتی بدون شلوغ کردن ناوبری اصلی.</p></div></div><div class="dashboard-quick-actions"><button class="quick-action" onclick="openBroadcast()"><span>↗</span><b>پیام همگانی</b></button><button class="quick-action" onclick="openPurchaseReward()"><span>★</span><b>پاداش خرید</b></button></div></article>`}
function renderAdminInfrastructure(){
  const I=adminState.infrastructure||{},c=I.counts||{},ps=I.providers||[],maps=adminState.service_plan_provider_maps||[],jobs=adminState.provisioning_jobs||[],mon=adminState.monitoring||{},routing=adminState.routing||{};
  return `<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🧩</span><div><h3>زیرساخت سرویس</h3><p class="muted">پنل‌ها، اتصال پلن‌ها و وضعیت خودکار سرویس‌ها</p></div></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(c.providers||0)}</b><span>پنل</span></div><div class="dq-stat"><b>${nf(c.online||0)}</b><span>آنلاین</span></div><div class="dq-stat"><b>${nf(c.services||0)}</b><span>سرویس</span></div><div class="dq-stat"><b>${nf(c.queued_jobs||0)}</b><span>در صف</span></div></div><div class="dashboard-quick-actions"><button class="quick-action" data-infra-refresh><span>↻</span><b>بروزرسانی</b></button><button class="quick-action" data-infra-run-monitor><span>♥</span><b>بررسی سلامت</b></button><button class="quick-action" data-infra-run-queue><span>⚙</span><b>اجرای صف</b></button></div></article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🌐</span><div><h3>پنل‌های متصل</h3><p class="muted">${nf(ps.length)} پنل تعریف شده</p></div></div>${ps.length?ps.map(p=>{const st=String(p.last_health_status||p.status||'unknown').toLowerCase();return `<div class="admin-list-row"><div><b>${esc(p.name||('پنل #'+p.id))}</b><small>${p.driver==='xui'?'3x-ui (Sanaei)':p.driver==='marzban'?'Marzban':p.driver==='remnawave'?'Remnawave':esc(p.driver||'')} · ${['online','healthy'].includes(st)?'آنلاین':st==='offline'?'آفلاین':st==='degraded'?'ناپایدار':'تست نشده'}</small></div><div><button class="secondary" data-infra-test="${p.id}">تست</button><button class="secondary" data-infra-toggle="${p.id}" data-active="${Number(p.is_active||0)}">${Number(p.is_active)?'غیرفعال':'فعال'}</button></div></div>`}).join(''):'<div class="u-empty"><h3>هنوز پنلی وصل نشده</h3><p>برای افزودن پنل و مپ‌کردن Inboundها از نسخه وب مدیریت استفاده کن.</p></div>'}</article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🧭</span><div><h3>اتصال پلن‌ها</h3><p class="muted">${nf(maps.length)} اتصال فعال/ثبت‌شده</p></div></div>${maps.slice(0,20).map(m=>{const ids=Array.isArray(m.config?.inbound_ids)?m.config.inbound_ids:[];const gb=Number(m.config?.traffic_limit_bytes||0)/1073741824;return `<div class="admin-list-row"><div><b>${esc(m.plan_title||('پلن #'+m.service_plan_id))}</b><small>${esc(m.provider_name||'پنل')} · ${ids.length?nf(ids.length)+' Inbound':'مقصد '+esc(m.remote_target_id||'—')} · ${gb?gb.toFixed(gb%1?1:0)+' GB':'بدون سقف'} · ${Number(m.config?.duration_days||0)?nf(m.config.duration_days)+' روز':'بدون محدودیت'}</small></div><span class="chip-mini ${Number(m.is_active)?'chip-active':'chip-off'}">${Number(m.is_active)?'فعال':'خاموش'}</span></div>`}).join('')||'<p class="muted">هنوز پلنی به پنل متصل نشده.</p>'}</article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">⚙</span><div><h3>صف ساخت و تمدید</h3><p class="muted">آخرین عملیات خودکار</p></div></div>${jobs.slice(0,15).map(j=>`<div class="admin-list-row"><div><b>#${nf(j.id)} · سفارش #${nf(j.order_id||0)}</b><small>${esc(j.provider_name||'پنل')} · ${j.status==='completed'?'کامل':j.status==='failed'?'ناموفق':'در انتظار'} · تلاش ${nf(j.attempt_count||0)}/${nf(j.max_attempts||0)}</small></div>${j.status!=='completed'?`<button class="secondary" data-infra-retry="${j.id}">تلاش دوباره</button>`:''}</div>`).join('')||'<p class="muted">صف خالی است.</p>'}</article>
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🚨</span><div><h3>سلامت و مسیریابی</h3><p class="muted">${nf(mon.stats?.open_incidents||0)} مشکل باز · ${nf(routing.stats?.open_circuits||0)} مدار موقتاً بسته</p></div></div></article>`;
}
function renderAdminAdvanced(){
  const A=adminState.advanced||{},pa=A.provider_analytics||[],mon=adminState.monitoring||{},routing=adminState.routing||{},inc=mon.open_incidents||[],runs=mon.recent_runs||[],decisions=routing.recent_decisions||[];
  const featureCards=[
    ['🧪','تست رایگان','پلن، حجم و زمان آزمایشی','data-adv-trial',''],
    ['➕','تمدید سراسری','حجم و روز اضافه برای همه پلن‌ها','data-adv-addons',''],
    ['♥','مانیتورینگ','Health check و Sync سرویس','data-adv-monitor',''],
    ['🧭','Failover','Routing و Circuit Breaker','data-adv-routing','']
  ];
  return `<article class="admin-card adv-clean-hero"><div class="admin-card-head"><span class="admin-card-icon">🚀</span><div><h3>امکانات پیشرفته</h3><p class="muted">هر قابلیت در یک کارت مشخص؛ جزئیات فنی فقط وقتی لازم باشد باز می‌شود.</p></div></div><div class="adv-feature-grid">${featureCards.map(([icon,title,desc,attr])=>`<button class="adv-feature-card" ${attr}><span>${icon}</span><div><b>${title}</b><small>${desc}</small></div></button>`).join('')}</div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(A.trials_active||0)}</b><span>تست فعال</span></div><div class="dq-stat"><b>${nf(mon.stats?.open_incidents||0)}</b><span>Incident باز</span></div><div class="dq-stat"><b>${nf(pa.length)}</b><span>پنل</span></div><div class="dq-stat"><b>${nf(routing.stats?.open_circuits||0)}</b><span>Circuit باز</span></div></div></article>
  ${pa.length?`<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">📈</span><div><h3>ظرفیت و هزینه Providerها</h3><p class="muted">تنظیمات عملیاتی هر پنل</p></div></div>${pa.map(p=>`<div class="admin-list-row"><div><b>${esc(p.name||'پنل')}</b><small>${p.driver==='xui'?'3x-ui':esc(p.driver||'')} · ${nf(p.active_services||0)} سرویس · ${(Number(p.traffic_used_bytes||0)/1073741824).toFixed(1)} GB</small></div><button class="secondary" data-adv-economics="${p.id}">تنظیم</button></div>`).join('')}</article>`:''}
  <article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🚨</span><div><h3>وضعیت اتوماسیون</h3><p class="muted">آخرین رخدادهای مهم</p></div></div><div class="adv-status-summary"><div><b>${inc.length?nf(inc.length):'✓'}</b><span>${inc.length?'Incident باز':'بدون Incident باز'}</span></div><div><b>${runs[0]?esc(runs[0].status||'—'):'—'}</b><span>آخرین Monitoring</span></div><div><b>${decisions.length?nf(decisions.length):'0'}</b><span>Routing اخیر</span></div></div>${inc.slice(0,5).map(x=>`<div class="admin-list-row"><div><b>${esc(x.provider_name||'Provider')}</b><small>${esc(x.message||x.title||'')}</small></div><span>${esc(x.opened_at||'')}</span></div>`).join('')}</article>`;
}
function openAdminAdvancedAddons(){const a=adminState.service_addons||{};openEdit('تمدید و افزونه سراسری',[{title:'برای همه پلن‌ها',fields:[{id:'adv_agb',label:'قیمت هر GB اضافه',type:'number',value:a.traffic_price_per_gb||0},{id:'adv_aday',label:'قیمت هر روز اضافه',type:'number',value:a.day_price||0},{id:'adv_mingb',label:'حداقل GB',type:'number',value:a.min_gb||1},{id:'adv_maxgb',label:'حداکثر GB',type:'number',value:a.max_gb||500},{id:'adv_minday',label:'حداقل روز',type:'number',value:a.min_days||1},{id:'adv_maxday',label:'حداکثر روز',type:'number',value:a.max_days||365}]}],async()=>adminAction('admin_service_addons_global',{traffic_price_per_gb:val('adv_agb'),day_price:val('adv_aday'),min_gb:val('adv_mingb'),max_gb:val('adv_maxgb'),min_days:val('adv_minday'),max_days:val('adv_maxday')}))}
function advancedPlanOptions(selected){const tree=adminState.catalog_admin?.tree||[];let out='<option value="">انتخاب پلن</option>';for(const sv of tree)for(const g of (sv.groups||[]))for(const p of (g.plans||[]))out+=`<option value="${p.id}" ${String(selected||'')===String(p.id)?'selected':''}>${esc(sv.name||'')} / ${esc(g.name||'')} / ${esc(p.title||'')}</option>`;return out}
function openAdminAdvancedTrial(){const a=adminState.advanced?.trial||{};openEdit('تنظیم تست رایگان',[{title:'Trial',fields:[{id:'adv_trial_on',label:'فعال (1/0)',type:'number',value:Number(a.enabled||0)},{id:'adv_trial_plan',label:'پلن تست',type:'select',options:advancedPlanOptions(a.service_plan_id)},{id:'adv_trial_gb',label:'حجم تست (GB)',type:'number',value:(Number(a.traffic_limit_bytes||1073741824)/1073741824).toFixed(2)},{id:'adv_trial_min',label:'مدت تست (دقیقه)',type:'number',value:a.duration_minutes||120},{id:'adv_trial_max',label:'حداکثر دفعات هر کاربر',type:'number',value:a.max_per_user||1},{id:'adv_trial_cd',label:'Cooldown (روز)',type:'number',value:a.cooldown_days||30}]}],async()=>adminAction('admin_trial_settings',{enabled:Number(val('adv_trial_on'))?1:0,service_plan_id:val('adv_trial_plan'),traffic_limit_bytes:Math.round(Number(val('adv_trial_gb')||0)*1073741824),duration_minutes:val('adv_trial_min'),max_per_user:val('adv_trial_max'),cooldown_days:val('adv_trial_cd')}))}
function openAdminAdvancedEconomics(id){const p=(adminState.advanced?.provider_analytics||[]).find(x=>Number(x.id)===Number(id))||{};openEdit('اقتصاد و ظرفیت پنل',[{title:p.name||'Provider',fields:[{id:'adv_cost',label:'هزینه ماهانه',type:'number',value:p.monthly_cost||0},{id:'adv_cur',label:'واحد هزینه',value:p.cost_currency||'USD'},{id:'adv_bw',label:'هزینه هر GB',type:'number',value:p.bandwidth_cost_per_gb||0},{id:'adv_maint',label:'حالت تعمیرات (1/0)',type:'number',value:Number(p.maintenance_mode||0)},{id:'adv_msg',label:'پیام تعمیرات',type:'textarea',value:p.maintenance_message||''},{id:'adv_cap',label:'آستانه ظرفیت نرم (%)',type:'number',value:p.soft_capacity_percent||90}]}],async()=>adminAction('admin_provider_economics',{provider_id:id,monthly_cost:val('adv_cost'),cost_currency:val('adv_cur'),bandwidth_cost_per_gb:val('adv_bw'),maintenance_mode:Number(val('adv_maint'))?1:0,maintenance_message:val('adv_msg'),soft_capacity_percent:val('adv_cap')}))}
function openAdminAdvancedRouting(){const r=adminState.routing?.settings||{};openEdit('تنظیم Failover',[{title:'Routing',fields:[{id:'adv_strategy',label:'Strategy',type:'select',options:`<option value="priority_weighted" ${r.strategy==='priority_weighted'?'selected':''}>اولویت + وزن</option><option value="priority" ${r.strategy==='priority'?'selected':''}>اولویت</option><option value="weighted" ${r.strategy==='weighted'?'selected':''}>وزن</option><option value="capacity" ${r.strategy==='capacity'?'selected':''}>ظرفیت</option>`},{id:'adv_reserve',label:'ظرفیت رزرو',type:'number',value:r.capacity_reserve||0},{id:'adv_fail',label:'خطا تا بازشدن Circuit',type:'number',value:r.circuit_failure_threshold||3},{id:'adv_open',label:'مدت Circuit (ثانیه)',type:'number',value:r.circuit_open_seconds||300},{id:'adv_degraded',label:'اجازه Provider ناپایدار (1/0)',type:'number',value:Number(r.allow_degraded??1)}]}],async()=>adminAction('admin_routing_settings',{strategy:val('adv_strategy'),capacity_reserve:val('adv_reserve'),circuit_failure_threshold:val('adv_fail'),circuit_open_seconds:val('adv_open'),allow_degraded:Number(val('adv_degraded'))?1:0}))}

function renderAdminDashboard(){const I=adminState.infrastructure||{},ps=I.providers||[],maps=adminState.service_plan_provider_maps||[],healthy=ps.filter(p=>['online','healthy'].includes(String(p.last_health_status||'').toLowerCase())).length,setupItems=[['پنل سرویس',ps.length>0,'infrastructure'],['اتصال سالم',healthy>0,'infrastructure'],['مپ پلن',maps.length>0,'infrastructure'],['اتوماسیون',!!adminState.monitoring?.settings,'advanced']],setupDone=setupItems.filter(x=>x[1]).length,onboarding=setupDone===setupItems.length?'':`<article class="admin-card admin-setup-card"><div class="admin-card-head"><span class="admin-card-icon">✓</span><div><h3>راه‌اندازی BlueGate · ${setupDone}/${setupItems.length}</h3><p class="muted">موارد باقی‌مانده را از همین‌جا کامل کن.</p></div></div><div class="admin-setup-list">${setupItems.map(([label,ok,route],i)=>`<button data-admin-tab="${route}" class="${ok?'done':''}"><span>${ok?'✓':i+1}</span><b>${label}</b></button>`).join('')}</div></article>`;const top=adminState.report?.top||[];const catalogTree=adminState.catalog_admin?.tree||[];const productCount=catalogTree.length;const variantCount=catalogTree.reduce((a,s)=>a+(s.groups||[]).reduce((b,g)=>b+(g.plans||[]).length,0),0);const orderCount=(adminState.orders||[]).length;const inventoryCount=(adminState.inventory||[]).length;const orders=adminState.orders||[];const rev7=last7DaysRevenue(orders);const total7=rev7.reduce((s,d)=>s+d.rev,0);const lowStock=(adminState.products||[]).filter(p=>Number(p.inventory_available||0)<3&&Number(p.is_active)).sort((a,b)=>Number(a.inventory_available||0)-Number(b.inventory_available||0));const topups=(adminState.credit_topups||[]).filter(x=>['receipt_submitted','reviewing'].includes(x.status));const topupReview=topups.length?`<article class="admin-card topup-admin-review"><div class="admin-card-head"><span class="admin-card-icon">💳</span><div><h3>شارژهای در انتظار بررسی</h3><p class="muted">${nf(topups.length)} درخواست نیازمند تصمیم</p></div></div><div class="topup-admin-list">${topups.slice(0,8).map(x=>`<div><span><b>#${nf(x.id)} · ${esc(x.first_name||x.username||'کاربر')}</b><small>${fmt(x.amount)} · ${esc(x.payment_method_fa||x.payment_method||'')}${x.tx_hash?' · TXID':''}${x.receipt_file_id?' · رسید':''}</small></span><div>${x.receipt_file_id?`<button class="secondary" data-credit-topup-receipt="${x.id}">مشاهده رسید</button>`:''}${x.tx_hash?`<button class="secondary" data-credit-topup-tx="${esc(x.tx_hash)}">TXID</button>`:''}<button class="success" data-credit-topup-approve="${x.id}">تایید</button><button class="danger" data-credit-topup-reject="${x.id}">رد</button></div></div>`).join('')}</div></article>`:'';return `${onboarding}${topupReview}<article class="admin-card dashboard-hero"><div class="admin-card-head"><span class="admin-card-icon">📊</span><div><h3>داشبورد فروش</h3><p class="muted">مرور سریع وضعیت فروشگاه و دسترسی به همه بخش‌ها.</p></div></div><div class="dashboard-quick-stats"><div class="dq-stat"><b>${nf(productCount)}</b><span>سرویس</span></div><div class="dq-stat"><b>${nf(variantCount)}</b><span>پلن</span></div><div class="dq-stat"><b>${nf(orderCount)}</b><span>سفارش</span></div><div class="dq-stat"><b>${nf(inventoryCount)}</b><span>آیتم انبار</span></div></div><div class="dashboard-quick-actions"><button class="quick-action" data-admin-tab="catalog"><span>🧭</span><b>Catalog Studio</b></button><button class="quick-action" data-admin-tab="orders"><span>🧾</span><b>سفارش‌ها</b></button><button class="quick-action" data-admin-tab="inventory"><span>📦</span><b>انبار</b></button><button class="quick-action" data-admin-tab="settings"><span>⚙️</span><b>تنظیمات</b></button><button class="quick-action" data-admin-tab="backups"><span>💾</span><b>بکاپ</b></button><button class="quick-action" data-admin-tab="notifications"><span>🔔</span><b>اعلان کاربران</b></button><button class="quick-action" onclick="openBroadcast()"><span>📢</span><b>پیام تلگرام</b></button><button class="quick-action" onclick="openPurchaseReward()"><span>🎁</span><b>پاداش خرید</b></button></div></article>${lowStock.length?`<article class="admin-card alert-card"><div class="admin-card-head"><span class="admin-card-icon">⚠️</span><div><h3>موجودی کم</h3><p class="muted">${nf(lowStock.length)} محصول کمتر از ۳ آیتم در انبار دارند.</p></div></div><div class="low-stock-list">${lowStock.slice(0,5).map(p=>`<div class="low-stock-row" data-admin-tab="inventory"><div><b>${esc(p.name)}</b><span class="muted">موجودی: ${nf(p.inventory_available||0)} آیتم</span></div><span class="chip-mini chip-${Number(p.inventory_available||0)===0?'off':'featured'}">${Number(p.inventory_available||0)===0?'ناموجود':'کم'}</span></div>`).join('')}</div>${lowStock.length>5?`<button class="secondary wide" data-admin-tab="inventory" style="margin-top:10px">مشاهده همه در انبار</button>`:''}</article>`:''}<article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">📈</span><div><h3>درآمد ۷ روز اخیر</h3><p class="muted">مجموع: ${fmt(total7)} تومان</p></div></div>${sparklineHtml(rev7)}</article>${(adminState.forecast&&adminState.forecast.forecast)?`<article class="admin-card forecast-card"><div class="admin-card-head"><span class="admin-card-icon">🔮</span><div><h3>پیش‌بینی ماه آینده</h3><p class="muted">بر اساس میانگین ۳۰ روز اخیر</p></div></div><div class="forecast-grid"><div class="forecast-main"><b>${fmt(adminState.forecast.forecast)}</b><span>تومان پیش‌بینی</span></div><div class="forecast-side"><span class="chip-mini chip-${adminState.forecast.change_percent>=0?'active':'off'}">${adminState.forecast.change_percent>=0?'▲':'▼'} ${nf(Math.abs(adminState.forecast.change_percent))}٪</span><small>نسبت به ماه قبل</small></div></div><p class="muted">میانگین روزانه: ${fmt(adminState.forecast.daily_avg)} تومان · ۳۰ روز اخیر: ${nf(adminState.forecast.last30_count)} سفارش</p></article>`:''}<div class="admin-charts-grid"><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🏆</span><div><h3>پرفروش‌ترین‌ها</h3><p class="muted">بر اساس تعداد سفارش</p></div></div>${top.length?barChartHtml(top):'<p class="muted empty-state">داده‌ای نیست.</p>'}</article><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🥧</span><div><h3>روش‌های پرداخت</h3><p class="muted">توزیع ${nf(orderCount)} سفارش</p></div></div>${pieChartHtml(orders)}</article></div>`}

function renderAdminNotifications(){const rows=adminState.notification_campaigns||[];const typeFa={info:'عادی',important:'مهم',promo:'تخفیف',service:'سرویس',security:'امنیتی'},audFa={all:'همه کاربران',service:'دارای سرویس',vpn:'کاربران VPN'};return `<article class="admin-card admin-notify-compose"><div class="admin-card-head"><span class="admin-card-icon">🔔</span><div><h3>ارسال اعلان به کاربران</h3><p class="muted">این پیام داخل Notification Center وب و Mini App ثبت می‌شود.</p></div></div><div class="admin-notify-form"><label><span>عنوان</span><input id="aniTitle" maxlength="255" placeholder="مثلاً لوکیشن جدید BluePing"></label><label><span>نوع</span><select id="aniType"><option value="info">عادی</option><option value="important">مهم</option><option value="promo">تخفیف</option><option value="service">سرویس</option><option value="security">امنیتی</option></select></label><label class="full"><span>متن پیام</span><textarea id="aniBody" maxlength="1000" placeholder="متن کوتاه و واضح اعلان"></textarea></label><label><span>مخاطب</span><select id="aniAudience"><option value="all">همه کاربران</option><option value="service">کاربران دارای سرویس</option><option value="vpn">کاربران VPN</option></select></label><label><span>با کلیک برو به</span><select id="aniAction"><option value="">بدون اکشن</option><option value="store">فروشگاه</option><option value="orders">سفارش‌ها</option><option value="wallet">اعتبار</option><option value="account">حساب</option><option value="service">سرویس مشخص</option><option value="url">لینک HTTPS</option></select></label><label class="full"><span>مقدار اکشن <small>اختیاری؛ برای سرویس Order ID و برای لینک URL</small></span><input id="aniActionValue" placeholder="مثلاً 123 یا https://..."></label><button class="primary wide" id="adminSendInAppNotification">ارسال اعلان</button></div></article><article class="admin-card"><div class="admin-card-head"><span class="admin-card-icon">🕘</span><div><h3>اعلان‌های ارسال‌شده</h3><p class="muted">${nf(rows.length)} کمپین اخیر</p></div></div><div class="admin-notification-history">${rows.length?rows.map(x=>`<div class="admin-notification-history-row"><span>${notificationIcon(x)}</span><div><b>${esc(x.title)}</b><small>${esc(x.body||'')}</small><em>${audFa[x.audience]||x.audience} · ${nf(x.recipient_count||0)} نفر · ${esc(String(x.created_at||'').slice(0,16))}</em></div><i>${typeFa[x.type]||x.type}</i></div>`).join(''):'<p class="muted empty-state">هنوز اعلانی ارسال نشده.</p>'}</div></article>`}
async function sendAdminInAppNotification(){const title=$('aniTitle')?.value.trim()||'',body=$('aniBody')?.value.trim()||'',type=$('aniType')?.value||'info',audience=$('aniAudience')?.value||'all',action_type=$('aniAction')?.value||'',action_value=$('aniActionValue')?.value.trim()||'';if(!title){showStatus('عنوان اعلان را وارد کن','error');return}try{showStatus('در حال ثبت اعلان…','info');adminState=await api('admin_notification_broadcast',{title,body,type,audience,action_type,action_value});renderAdmin();showStatus(adminState.message||'اعلان ارسال شد','success')}catch(e){showStatus(e.message||'ارسال اعلان انجام نشد','error')}}
...
function mcUploadField(kind,key='',url=''){
  const keyAttr=key?` data-key="${esc(key)}"`:'';
  return `<div class="mcw-upload-card full ${url?'has-image':''}">
    <div class="mcw-upload-main">
      <div class="mcw-upload-icon">${url?`<img src="${esc(url)}" alt="">`:'🖼️'}</div>
      <div><b>${url?'تصویر فعلی':'آپلود تصویر'}</b><small>JPG / PNG / WEBP · حداکثر ۶MB</small></div>
    </div>
    <label class="mcw-upload-action"><input type="file" accept="image/jpeg,image/png,image/webp" data-mc-upload="${kind}"${keyAttr}><span>${url?'تغییر تصویر':'انتخاب تصویر'}</span></label>
  </div>`;
}
function mcSettingRow(label,desc,field,checked,icon='●'){
  return `<label class="mcw-setting-row"><span class="mcw-setting-icon">${icon}</span><span class="mcw-setting-copy"><b>${label}</b><small>${desc}</small></span><span class="mcw-toggle"><input type="checkbox" ${field} ${checked?'checked':''}><i></i></span></label>`;
}
function mcGroupEditorHtml(g){
  return `<div class="form-grid mcw-grid compact">
    <label class="mcw-field"><span>نام</span><input data-mcgf="name" value="${esc(g.name)}"></label>
    <label class="mcw-field"><span>Slug</span><input data-mcgf="slug" value="${esc(g.slug||'')}"></label>
    <label class="mcw-field full"><span>توضیح</span><input data-mcgf="description" value="${esc(g.description)}"></label>
    <label class="mcw-field"><span>ترتیب</span><input data-mcgf="sort_order" type="number" value="${Number(g.sort_order||0)}"></label>
    <label class="mcw-field full"><span>لینک تصویر</span><input data-mcgf="image_url" value="${esc(g.image_url||'')}"></label>
    ${mcUploadField('group',g.key,g.image_url)}
  </div><div class="mcw-inline-setting">${mcSettingRow('فعال','این زیرسرویس قابل انتخاب باشد','data-mcgf="is_active"',Number(g.is_active),'●')}</div>`;
}
function mcPlanEditorHtml(p){
  return `<div class="form-grid mcw-grid compact">
    <label class="mcw-field full"><span>عنوان</span><input data-mcpf="title" value="${esc(p.title)}"></label>
    <label class="mcw-field"><span>واحد قیمت</span><select data-mcpf="price_currency"><option value="IRT" ${p.price_currency==='IRT'?'selected':''}>تومان</option><option value="USD" ${p.price_currency==='USD'?'selected':''}>USD → تومان خودکار</option></select></label>
    <label class="mcw-field"><span>قیمت تومان</span><input data-mcpf="price" type="number" value="${Number(p.price||0)}"></label>
    <label class="mcw-field"><span>قیمت USD</span><input data-mcpf="price_usd" type="number" step="0.0001" value="${esc(p.price_usd??'')}"></label>
    <label class="mcw-field"><span>مدت (روز)</span><input data-mcpf="duration_days" type="number" value="${Number(p.duration_days||0)}"></label>
    <label class="mcw-field"><span>تخفیف ٪</span><input data-mcpf="discount_percent" type="number" value="${Number(p.discount_percent||0)}"></label>
    <label class="mcw-field"><span>نوع تحویل</span><select data-mcpf="delivery_type">${['manual','account','vpn','code','file'].map(x=>`<option value="${x}" ${p.delivery_type===x?'selected':''}>${x}</option>`).join('')}</select></label>
    <label class="mcw-field"><span>نوع پورسانت</span><select data-mcpf="commission_type">${['none','percent','fixed'].map(x=>`<option value="${x}" ${p.commission_type===x?'selected':''}>${x}</option>`).join('')}</select></label>
    <label class="mcw-field"><span>مقدار پورسانت</span><input data-mcpf="commission_value" type="number" value="${Number(p.commission_value||0)}"></label>
    <label class="mcw-field"><span>ترتیب</span><input data-mcpf="sort_order" type="number" value="${Number(p.sort_order||0)}"></label>
    <label class="mcw-field full"><span>توضیح</span><input data-mcpf="description" value="${esc(p.description)}"></label>
    <label class="mcw-field full"><span>لینک تصویر</span><input data-mcpf="image_url" value="${esc(p.image_url||'')}"></label>
    ${mcUploadField('plan',p.key,p.image_url)}
  </div><div class="mcw-inline-setting">${mcSettingRow('فعال','این پلن در انتخاب‌های کاربر نمایش داده شود','data-mcpf="is_active"',Number(p.is_active),'●')}</div>`;
}
function mcSyncLazyField(el){
  const w=mobileCatalogWizard;if(!w||!el)return;
  const value=el.type==='checkbox'?(el.checked?1:0):(el.type==='number'?Number(el.value||0):el.value);
  if(el.dataset.mcgf){const box=el.closest('[data-mcg]');const g=box&&w.groups.find(x=>x.key===box.dataset.mcg);if(g)g[el.dataset.mcgf]=value}
  if(el.dataset.mcpf){const box=el.closest('[data-mcp]');const p=box&&w.groups.flatMap(g=>g.plans||[]).find(x=>x.key===box.dataset.mcp);if(p)p[el.dataset.mcpf]=value}
}
async function mcHandleUploadInput(inp,w){
  if(!inp?.files?.[0])return;const wizard=w;mcCollect();mobileCatalogUploadPending++;mcUploadBusy(true);inp.disabled=true;
  try{showStatus('در حال آپلود تصویر…');const url=await mcUploadImage(inp.files[0]);if(mobileCatalogWizard!==wizard)return;const kind=inp.dataset.mcUpload,key=inp.dataset.key;if(kind==='service')wizard.image_url=url;else if(kind==='group'){const g=wizard.groups.find(x=>x.key===key);if(g)g.image_url=url}else if(kind==='plan'){const p=wizard.groups.flatMap(g=>g.plans||[]).find(x=>x.key===key);if(p)p.image_url=url}try{wizard._draft_schema=MOBILE_CATALOG_DRAFT_SCHEMA;localStorage.setItem(`nx_mcw_${wizard.id||'new'}`,JSON.stringify(wizard))}catch(_){}renderCatalogMobileWizard();showStatus('تصویر آپلود شد','success')}catch(e){showStatus(e.message||'آپلود تصویر ناموفق بود','error')}finally{mobileCatalogUploadPending=Math.max(0,mobileCatalogUploadPending-1);mcUploadBusy(mobileCatalogUploadPending>0);if(inp?.isConnected)inp.disabled=false}
}
function mcStepHtml(){
  const w=mobileCatalogWizard;
  if(mobileCatalogStep===0)return `<div class="mcw-section-head"><span class="mcw-section-kicker">اطلاعات اصلی</span><h3>${w.id?'ویرایش مشخصات سرویس':'سرویس جدید'}</h3><p>اطلاعات اصلی و ظاهر ویترین را بدون حذف هیچ تنظیمی مدیریت کن.</p></div>
    <section class="mcw-card"><div class="mcw-card-title"><span>✦</span><div><b>مشخصات سرویس</b><small>اطلاعاتی که در کاتالوگ و URL استفاده می‌شود</small></div></div><div class="form-grid mcw-grid compact">
      <label class="mcw-field full"><span>نام سرویس</span><input data-mcw="name" value="${esc(w.name)}" placeholder="مثلاً BluePing"></label>
      <label class="mcw-field"><span>Slug</span><input data-mcw="slug" value="${esc(w.slug||'')}" placeholder="blueping"></label>
      <label class="mcw-field"><span>دسته</span><select data-mcw="category_id">${catalogCategoryOptions(w.category_id)}</select></label>
      <label class="mcw-field"><span>ترتیب</span><input data-mcw="sort_order" type="number" value="${Number(w.sort_order||0)}"></label>
      <label class="mcw-field"><span>Badge</span><input data-mcw="badge" value="${esc(w.badge)}" placeholder="اختیاری"></label>
    </div></section>
    <section class="mcw-card"><div class="mcw-card-title"><span>◈</span><div><b>ظاهر و توضیحات</b><small>تمام گزینه‌های قبلی حفظ شده‌اند</small></div></div><div class="form-grid mcw-grid compact">
      <label class="mcw-field"><span>تم کارت</span><select data-mcw="theme">${['blue','green','purple','gold'].map(x=>`<option value="${x}" ${w.theme===x?'selected':''}>${x}</option>`).join('')}</select></label>
      <label class="mcw-field full"><span>توضیح کوتاه</span><textarea data-mcw="description" rows="3">${esc(w.description)}</textarea></label>
      <label class="mcw-field full"><span>لینک تصویر</span><input data-mcw="image_url" value="${esc(w.image_url)}" placeholder="https://..."></label>
      ${mcUploadField('service','',w.image_url)}
    </div></section>
    <section class="mcw-card mcw-settings-card"><div class="mcw-card-title"><span>⚙</span><div><b>وضعیت نمایش</b><small>کنترل انتشار سرویس در فروشگاه</small></div></div><div class="mcw-setting-list">
      ${mcSettingRow('ویژه','نمایش برجسته‌تر در بخش‌های پیشنهادی','data-mcw="is_featured"',Number(w.is_featured),'★')}
      ${mcSettingRow('فعال','سرویس برای کاربران قابل مشاهده و خرید باشد','data-mcw="is_active"',Number(w.is_active),'●')}
    </div></section>`;

  if(mobileCatalogStep===1)return `<div class="mcw-section-head"><span class="mcw-section-kicker">ساختار</span><h3>مشتری چطور پلن را انتخاب کند؟</h3><p>ساختار فقط نحوه نمایش را تعیین می‌کند؛ همه تنظیمات سرویس حفظ می‌شوند.</p></div><div class="mcw-mode mcw-mode-clean">
    <button type="button" class="${w.mode==='direct'?'active':''}" data-mc-mode="direct"><span>⚡</span><b>پلن مستقیم</b><small>مناسب سرویس‌هایی که کاربر مستقیماً ۱ ماهه، ۳ ماهه و… را انتخاب می‌کند.</small><i></i></button>
    <button type="button" class="${w.mode==='grouped'?'active':''}" data-mc-mode="grouped"><span>🧩</span><b>زیرسرویس + پلن</b><small>مناسب Standard / Pro یا هر سرویسی که یک انتخاب قبل از پلن دارد.</small><i></i></button>
  </div>`;

  if(mobileCatalogStep===2){
    if(w.mode==='direct')return `<div class="mcw-section-head"><span class="mcw-section-kicker">زیرسرویس</span><h3>این مرحله لازم نیست</h3><p>برای ساختار مستقیم، Default Group پشت صحنه حفظ و مدیریت می‌شود.</p></div><div class="mcw-empty mcw-empty-clean"><span>⚡</span><b>ساختار مستقیم آماده است</b><p>بدون حذف هیچ داده‌ای می‌توانی مستقیم به پلن‌ها بروی.</p></div>`;
    const groups=w.groups.filter(g=>!g.is_default);
    return `<div class="mcw-section-head"><span class="mcw-section-kicker">زیرسرویس‌ها</span><h3>گروه‌های سرویس</h3><p>جزئیات هر زیرسرویس فقط وقتی بازش کنی ساخته می‌شود؛ هیچ فیلدی حذف نشده.</p></div><div class="mcw-list mcw-lazy-list">${groups.map((g,i)=>`<article class="mcw-lazy-card" data-mcg="${g.key}"><button type="button" class="mcw-lazy-summary" data-mc-toggle-group="${g.key}"><span class="mcw-lazy-index">${i+1}</span><div><b>${esc(g.name||'زیرسرویس جدید')}</b><small>${esc(g.slug||'بدون slug')} · ${nf((g.plans||[]).length)} پلن · ${Number(g.is_active)?'فعال':'غیرفعال'}</small></div><i>⌄</i></button><div class="mcw-lazy-body" data-mc-group-body="${g.key}" hidden></div><button type="button" class="mcw-lazy-remove danger ghost" data-mc-remove-group="${g.key}">حذف زیرسرویس</button></article>`).join('')}</div><button class="secondary wide mcw-add-row" data-mc-add-group>＋ افزودن زیرسرویس</button>`;
  }

  if(mobileCatalogStep===3){
    let gs=w.mode==='direct'?w.groups.filter(g=>g.is_default):w.groups.filter(g=>!g.is_default||g.plans.length);
    if(w.mode==='direct'&&!gs.length){const g=mcGroup({is_default:1});w.groups.unshift(g);gs=[g]}
    return `<div class="mcw-section-head"><span class="mcw-section-kicker">پلن‌ها</span><h3>قیمت و نحوه تحویل</h3><p>پلن‌ها به‌صورت کشویی و Lazy هستند؛ فرم کامل هر پلن فقط هنگام بازکردن ساخته می‌شود.</p></div><div class="mcw-plan-groups">${gs.map(g=>`<section class="mcw-plan-group-clean"><header><div><b>${g.is_default?'⚡ پلن‌های مستقیم':'🧩 '+esc(g.name)}</b><small>${nf((g.plans||[]).length)} پلن</small></div><button type="button" class="secondary" data-mc-add-plan="${g.key}">＋ پلن</button></header><div class="mcw-plan-list mcw-lazy-list">${(g.plans||[]).map((p,idx)=>`<article class="mcw-lazy-card mcw-lazy-plan" data-mcp="${p.key}"><button type="button" class="mcw-lazy-summary" data-mc-toggle-plan="${p.key}"><span class="mcw-lazy-index">${idx+1}</span><div><b>${esc(p.title||'پلن جدید')}</b><small>${p.price_currency==='USD'?'$'+nf(p.price_usd||0):fmt(p.price)} · ${Number(p.duration_days||0)} روز · ${esc(p.delivery_type||'manual')} · ${Number(p.is_active)?'فعال':'غیرفعال'}</small></div><i>⌄</i></button><div class="mcw-lazy-body" data-mc-plan-body="${p.key}" hidden></div><button type="button" class="mcw-lazy-remove danger ghost" data-mc-remove-plan="${p.key}" data-group-key="${g.key}">حذف پلن</button></article>`).join('')||'<p class="muted mcw-no-plan">هنوز پلنی نیست.</p>'}</div></section>`).join('')}</div>`;
  }

  const gs=w.mode==='direct'?w.groups.filter(g=>g.is_default):w.groups.filter(g=>!g.is_default);
  return `<div class="mcw-section-head"><span class="mcw-section-kicker">بررسی نهایی</span><h3>قبل از ذخیره یک نگاه آخر</h3><p>ذخیره، موارد حذف‌شده موجود را طبق رفتار فعلی سیستم به‌صورت امن غیرفعال می‌کند.</p></div><div class="mcw-preview mcw-preview-clean"><header>${w.image_url?`<img src="${esc(w.image_url)}" alt="">`:`<span>${esc((catalogAdminData().categories||[]).find(c=>Number(c.id)===Number(w.category_id))?.emoji||'🛍️')}</span>`}<div><small>${esc((catalogAdminData().categories||[]).find(c=>Number(c.id)===Number(w.category_id))?.title||'دسته')}</small><h3>${esc(w.name||'نام سرویس')}</h3><p>${esc(w.description||'بدون توضیح')}</p></div></header><div class="mcw-review-chips"><span>${w.mode==='direct'?'⚡ پلن مستقیم':'🧩 ساختار گروهی'}</span><span>${Number(w.is_active)?'● فعال':'○ غیرفعال'}</span>${Number(w.is_featured)?'<span>★ ویژه</span>':''}</div>${gs.map(g=>`<section class="mcw-review-group"><b>${g.is_default?'پلن‌ها':esc(g.name)}</b><div>${(g.plans||[]).map(p=>`<span><strong>${esc(p.title)}</strong><small>${p.price_currency==='USD'?'$'+nf(p.price_usd||0):fmt(p.price)}</small></span>`).join('')||'<em>بدون پلن</em>'}</div></section>`).join('')}</div>`;
}
function renderCatalogMobileWizard(){
  ...
  BlueGateUI.openSheet({type:'fullscreen',eyebrow:'CATALOG STUDIO',title:w.id?'ویرایش سرویس':'سرویس جدید',subtitle:labels[mobileCatalogStep],body,footer,onOpen:(sheet)=>{
    ...
    const syncLazy=(ev)=>{const el=ev.target;if(el?.matches?.('[data-mcgf],[data-mcpf]'))mcSyncLazyField(el)};
    sheet.addEventListener('input',syncLazy);sheet.addEventListener('change',syncLazy);
    sheet.addEventListener('change',ev=>{const inp=ev.target.closest?.('[data-mc-upload]');if(inp)mcHandleUploadInput(inp,w)});
    sheet.querySelectorAll('[data-mc-toggle-group]').forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.mcToggleGroup,body=sheet.querySelector(`[data-mc-group-body="${key}"]`),card=btn.closest('.mcw-lazy-card'),g=w.groups.find(x=>x.key===key);if(!body||!g)return;const opening=body.hidden;if(opening&&!body.dataset.loaded){body.innerHTML=mcGroupEditorHtml(g);body.dataset.loaded='1'}body.hidden=!opening;card?.classList.toggle('open',opening)}));
    sheet.querySelectorAll('[data-mc-toggle-plan]').forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.mcTogglePlan,body=sheet.querySelector(`[data-mc-plan-body="${key}"]`),card=btn.closest('.mcw-lazy-card'),p=w.groups.flatMap(g=>g.plans||[]).find(x=>x.key===key);if(!body||!p)return;const opening=body.hidden;if(opening&&!body.dataset.loaded){body.innerHTML=mcPlanEditorHtml(p);body.dataset.loaded='1'}body.hidden=!opening;card?.classList.toggle('open',opening)}));
...
function walletById(id){id=Number(id||0);return (adminUiWallets||[]).find(x=>Number(x.id||0)===id)||(adminState?.settings?.crypto_wallets||[]).find(x=>Number(x.id||0)===id)||null}
function walletIndexById(id){id=Number(id||0);return (adminUiWallets||[]).findIndex(x=>Number(x.id||0)===id)}
  ...
  return `<div class="builder-list">`+items.map((it,i)=>{
    ...
    if(type==='wallet') {
      ...
      return `<article class="crypto-wallet-card-mini ${isActive?'':'is-off'}">
        ...
        <div class="crypto-wallet-actions-mini"><button class="secondary" data-wallet-edit-id="${Number(it.id||0)}" data-builder-edit="wallet:${i}">✏️ ویرایش</button><button class="secondary ghost" data-copy-wallet-address="${esc(addr)}">کپی آدرس</button><button class="danger ghost" data-wallet-delete-id="${Number(it.id||0)}" data-builder-del="wallet:${i}">حذف</button></div>
      </article>`;
    }
    return `<div class="builder-row compact-row">
      <div class="builder-row-info">
  ...
  return `<section class="settings-dashboard better-settings">
    ...
    <div class="settings-subtab-pane ${isPay?'':'hidden'}" data-pane="payments"><button class="crypto-wallet-shortcut" data-settings-subtab="crypto"><span>🪙</span><div><b>کیف پول‌های رمزارز</b><small>${nf((s.crypto_wallets||[]).length)} Wallet ثبت‌شده · مدیریت و ویرایش</small></div><i>‹</i></button>
      <article class="settings-card admin-card">
        ...
        <div class="form-grid settings-form" style="margin-top:14px">
          <label><span>حالت نرخ Stars</span><select id="as_stars_mode"><option value="auto" ${s.stars_rate_mode!=='manual'?'selected':''}>خودکار با USDT</option><option value="manual" ${s.stars_rate_mode==='manual'?'selected':''}>دستی</option></select></label><label><span>Base هر Star (USDT)</span><input id="as_stars_base_usdt" value="${esc(s.stars_base_usdt||0.016)}" inputmode="decimal"></label><label><span>Markup Stars %</span><input id="as_stars_markup" value="${esc(s.stars_rate_markup_percent||0)}" inputmode="decimal"></label><label><span>Fallback دستی تومان</span><input id="as_stars_rate" value="${esc(s.stars_rate_manual_toman||3200)}" inputmode="numeric"></label><div class="full rate-live-box"><b>⭐ نرخ فعلی: ${nf(s.stars_rate_toman||0)} تومان / Star</b><small>${esc(s.stars_rate_meta?.source||'manual')} · USDT ${nf(s.stars_rate_meta?.usdt_rate||0)} تومان</small></div>
    ...
    <div class="settings-subtab-pane ${isCry?'':'hidden'}" data-pane="crypto">
      <article class="settings-card admin-card builder-card">
        <div class="admin-card-head crypto-wallet-admin-head"><div class="admin-card-icon"><span>🪙</span></div><div><h3>کیف پول‌های رمزارز</h3><p class="muted">${nf((s.crypto_wallets||[]).length)} Wallet ثبت‌شده · Walletهای واقعی دیتابیس پایین نمایش داده می‌شوند.</p></div><button type="button" class="secondary crypto-wallet-refresh-mini" data-refresh-wallet-list>↻ بروزرسانی</button></div>
        ...
        <div id="walletBuilderList">${cryptoWalletDbCardsHtml()}</div>
...
async function adminAction(action,payload={}){
  try{
    ...
    if(!result || result.ok===false) throw new Error(result?.message||'خطا در ذخیره');
    // Many infrastructure/advanced endpoints intentionally return a partial payload.
    // Always re-hydrate the complete admin state after mutations so Mini App controls stay functional.
    adminState=action==='admin_summary'?result:await api('admin_summary');
...
async function loadAfterAction(action,payload={}){try{state=await api(action,payload);const active=topMiniLayer();if(active==='appSheet')BlueGateUI?.closeSheet?.();applyTheme(state);renderUser();showStatus('انجام شد');syncTelegramBackButton();return true}catch(e){showStatus(e.message,'error');return false}}

document.addEventListener('click',async(e)=>{
  const b=e.target.closest('[data-builder-add],[data-builder-edit],[data-builder-del],[data-wallet-edit-id],[data-wallet-delete-id],[data-copy-wallet-address],[data-refresh-wallet-list],[data-admin-color],#applyCustomColor,#applyAdminColor,[data-close-share],[data-share-tg-url],[data-share-copy-url],[data-share-native],[data-share-product],[data-wishlist-pid]');
  if(!b) return;
  ...
  if(b.dataset.copyWalletAddress!==undefined){copyText(b.dataset.copyWalletAddress||'');showStatus('آدرس Wallet کپی شد');return;}
  if(b.dataset.walletEditId!==undefined){const i=walletIndexById(b.dataset.walletEditId);if(i<0){showStatus('Wallet در لیست فعلی پیدا نشد؛ لیست را بروزرسانی کن.','error');return}openWalletBuilder(i);return;}
  if(b.dataset.walletDeleteId!==undefined){const id=Number(b.dataset.walletDeleteId||0);if(!id)return showStatus('شناسه Wallet معتبر نیست','error');if(!await BlueGateUI.confirm({title:'حذف Wallet',message:'این Wallet از دیتابیس حذف شود؟',confirmText:'حذف',danger:true}))return;try{const r=await api('admin_crypto_wallet_delete',{wallet_id:id});adminState.settings.crypto_wallets=r.wallets||[];adminState.settings.crypto_wallets_text=r.crypto_wallets_text||'';parseSettingsBuilders();renderAdmin();showStatus('Wallet حذف شد')}catch(err){showStatus(err.message||'حذف Wallet انجام نشد','error')}return;}
  if(b.dataset.refreshWalletList!==undefined){try{adminState=await api('admin_summary');parseSettingsBuilders();renderAdmin();showStatus('Walletها از دیتابیس بروزرسانی شدند')}catch(err){showStatus(err.message||'بروزرسانی Walletها ناموفق بود','error')}return;}
...
document.addEventListener('click',async(e)=>{
  ...
  const t=e.target.closest('button,a,[data-admin-tab],[data-settings-subtab],[data-admin-view-mode],[data-admin-action-sheet],[data-tab],[data-tab-jump],[data-color],[data-cat],[data-shop-sort],[data-shop-toggle],[data-clear-filters],[data-product],[data-product-preview],[data-back-shop],[data-wallet-order],[data-select-card],[data-pay-stars],[data-select-crypto],[data-select-crypto-tab],[data-reset-payment-method],[data-show-crypto],[data-crypto-hash],[data-check-crypto],[data-bulk-action],[data-reorder],[data-chat-user],[data-customer-360],[data-customer360],[data-edit-role],[data-admin-remove-role],[data-contact-wallet],[data-contact-ban],[data-edit-inventory],[data-admin-delete-inventory],[data-admin-hard-delete-inventory],[data-admin-status],[data-admin-order-note],[data-admin-archive-order],[data-admin-delete-order],[data-admin-cleanup],[data-admin-deliver],[data-admin-service],[data-admin-provision],[data-service-view],[data-service-copy-link],[data-view-receipt],[data-admin-save-settings],[data-open-url],[data-copy],[data-receipt],[data-customer-note],[data-order-filter],[data-order-open],[data-order-back],[data-order-more],[data-hide-order],[data-clear-canceled],[data-cancel],[data-refresh-crypto-rates],[data-admin-backup-create],[data-admin-backup-sendbot],[data-admin-backup-delete],[data-admin-backup-restore-server],[data-admin-backup-upload],[data-admin-load-more-orders],[data-accordion-toggle],[data-accordion-add-variant],[data-edit-coupon],[data-admin-toggle-coupon],[data-admin-delete-coupon],[data-credit-topup-approve],[data-credit-topup-reject],[data-credit-topup-receipt],[data-credit-topup-tx],[data-mini-topup-change],[data-mini-topup-cancel],[data-renew-order],[data-my-service],[data-my-service-back],[data-vpn-detail-back],[data-service-renew],[data-service-sync],[data-service-add-traffic],[data-service-add-days],[data-service-qr],[data-support-ticket],[data-support-ticket-new],[data-support-ticket-reply],[data-service-history],[data-service-events],[data-adv-trial],[data-adv-addons],[data-adv-monitor],[data-adv-routing],[data-adv-economics]')||e.target;if(t.id==='openShopFilters'){openShopFilters();return}if(t.id==='orderAdvancedFilter'){openOrderFiltersSheet();return}if(t.id==='ordersManageBtn'){openOrdersManageSheet();return}if(t.dataset.orderMore){openOrderMoreActions(t.dataset.orderMore);return}if(t.dataset.creditView){openCreditSubview(t.dataset.creditView);return}if(t.dataset.serviceView){openDirectServiceViewer(t.dataset.serviceView,t.dataset.serviceUrl||'',t.dataset.serviceTitle||'مدیریت سرویس');return}if(t.dataset.serviceCopyLink){copyMiniServiceUrl(t.dataset.serviceCopyLink,t.dataset.serviceUrl||'');return}if(t.dataset.settingsSubtab){if(typeof haptic==='function')haptic('light');settingsSubTab=t.dataset.settingsSubtab;document.querySelectorAll('.settings-subtab-btn').forEach(btn=>btn.classList.toggle('active',btn.dataset.settingsSubtab===settingsSubTab));document.querySelectorAll('.settings-subtab-pane').forEach(pane=>pane.classList.toggle('hidden',pane.dataset.pane!==settingsSubTab));if(settingsSubTab==='crypto'){parseSettingsBuilders();syncPaymentBuilders();}const settingsTabLabels = { general: '🏪 عمومی', payments: '💳 روش‌های پرداخت', crypto: '🪙 کریپتو & ارزها', appearance: '🎨 ظاهر & تم', gamification: '🎡 پاداش & گردونه' };const lbl=$('activeSettingsTabLabel');if(lbl)lbl.textContent=settingsTabLabels[settingsSubTab]||'';const muted=$('activeSettingsTabMuted');if(muted)muted.textContent=settingsTabLabels[settingsSubTab]||'';return}if(t.dataset.adminViewMode){adminOrderViewMode=t.dataset.adminViewMode;renderAdmin();return}if(t.dataset.adminActionSheet){const [type,id]=t.dataset.adminActionSheet.split(':');openAdminActionSheet(type,id);return}if(t.dataset.tab){setTab(t.dataset.tab)}if(t.dataset.tabJump){setTab(t.dataset.tabJump)}if(t.id==='paletteQuick'){openPalettePopup();return}if(t.id==='editMiniProfile'||t.id==='editMiniProfile2'||t.id==='completeMiniSecurity'){openMiniProfileEditor();return}if(t.id==='miniChangeEmailBtn'||t.id==='miniChangeEmailInlineBtn'){openMiniEmailChangeFlow();return}if(t.id==='changeMiniPassword'){openMiniPasswordEditor();return}if(t.id==='shareInviteHome'){const link=state.user?.referral_link||'';if(link){if(TG?.openTelegramLink)TG.openTelegramLink('https://t.me/share/url?url='+encodeURIComponent(link));else navigator.clipboard?.writeText(link);showStatus('لینک دعوت آماده اشتراک است')}return}if(t.id==='miniReferralJump'){openCreditSubview('referral');return}if(['miniCreditTopup','miniCreditTopup2','miniCreditTopupEmpty','miniCreditTopupHistory','miniCreditTopupHistoryEmpty','miniCreditTopupHome'].includes(t.id)){openMiniCreditTopup();return}if(t.dataset.miniTopupCancel){if(await BlueGateUI.confirm({title:'لغو درخواست شارژ',message:'این درخواست شارژ لغو شود؟ اگر رسید یا TXID فرستاده‌ای، دیگر بررسی نخواهد شد.',confirmText:'لغو درخواست',danger:true})){try{state=await api('cancel_credit_topup',{topup_id:Number(t.dataset.miniTopupCancel)});renderWallet();showStatus('درخواست شارژ لغو شد')}catch(e){showStatus(e.message||'لغو درخواست انجام نشد','error')}}return}if(t.dataset.miniTopupChange){if(await BlueGateUI.confirm({title:'تغییر روش پرداخت',message:'درخواست فعلی بسته می‌شود و یک درخواست جدید با همان مبلغ برای انتخاب روش تازه ساخته می‌شود.',confirmText:'تغییر روش'})){try{const r=await api('credit_topup_change_method',{topup_id:Number(t.dataset.miniTopupChange)});state=r;renderWallet();openMiniTopupMethods(r.topup);showStatus('روش پرداخت جدید را انتخاب کن')}catch(e){showStatus(e.message||'تغییر روش پرداخت انجام نشد','error')}}return}if(t.id==='miniTrialBtn'){if(!await BlueGateUI.confirm({title:'فعال‌سازی تست رایگان',message:'تست رایگان روی یکی از Providerهای سالم BlueGate ساخته شود؟',confirmText:'فعال کن'}))return;try{const r=await api('trial_request',{});if(r.service){state.services=[r.service,...(state.services||[]).filter(x=>Number(x.id)!==Number(r.service.id))]}state.trial=r.trial||{eligible:false};renderUser();showStatus('تست رایگان فعال شد','success')}catch(e){showStatus(e.message||'ساخت تست رایگان انجام نشد','error')}return}if(t.dataset.supportTicketNew!==undefined){newMiniSupportTicket();return}if(t.dataset.supportTicket){openMiniSupportTicket(Number(t.dataset.supportTicket));return}if(t.dataset.supportTicketReply){const id=Number(t.dataset.supportTicketReply);openDialog('پاسخ به تیکت','پیامت را بنویس.','متن پاسخ',async(message)=>{if(!message)return;try{await api('support_ticket_reply',{ticket_id:id,message});showStatus('پاسخ ارسال شد','success');openMiniSupportTicket(id)}catch(e){showStatus(e.message||'ارسال پاسخ ناموفق بود','error')}});return}if(t.id==='miniSupportBtn'){openMiniSupport();return}if(t.id==='miniNotificationsBtn'){openNotifications();return}if(t.dataset.myService){currentServiceOrderId=Number(t.dataset.myService);currentTab='home';renderUser();miniScrollTop();return}if(t.dataset.myServiceBack!==undefined){currentServiceOrderId=null;currentTab='home';renderUser();miniScrollTop();return}if(t.dataset.vpnDetailBack!==undefined){if(t.dataset.vpnDetailBack==='orders'){currentOrderId=null;currentTab='orders'}else{currentServiceOrderId=null;currentTab='home'}renderUser();miniScrollTop();return}if(t.dataset.serviceQr){const u=String(t.dataset.serviceUrl||'');if(u){const q='https://quickchart.io/qr?size=320&text='+encodeURIComponent(u);if(TG?.openLink)TG.openLink(q);else window.open(q,'_blank','noopener')}return}if(t.dataset.serviceHistory){try{const r=await api('service_usage_history',{service_id:Number(t.dataset.serviceHistory),days:30});prepareMiniLayer('previewSheet');const pv=$('previewSheet');const rows=r.history||[];const max=Math.max(1,...rows.map(x=>Number(x.traffic_used_bytes||0)));pv.innerHTML=`<div class="preview-sheet-inner"><div class="preview-sheet-handle" data-close-preview></div><h3>📈 مصرف ۳۰ روز اخیر</h3><div style="display:flex;align-items:end;gap:5px;height:190px;margin:20px 0;overflow-x:auto">${rows.map(x=>`<div title="${esc(x.created_at)}" style="min-width:12px;height:${Math.max(4,Math.round(Number(x.traffic_used_bytes||0)*170/max))}px;background:var(--tg-theme-button-color,#4c8dff);border-radius:5px"></div>`).join('')||'<div class="u-empty">هنوز Snapshot مصرف ثبت نشده.</div>'}</div><div class="bg-admin-list">${rows.slice(-12).reverse().map(x=>`<div class="account-menu-row"><div><b>${esc(x.created_at)}</b><small>${(Number(x.traffic_used_bytes||0)/1073741824).toFixed(2)} GB</small></div></div>`).join('')}</div></div>`;pv.classList.add('open');pv.querySelectorAll('[data-close-preview]').forEach(x=>x.addEventListener('click',closePreviewSheet))}catch(e){showStatus(e.message||'تاریخچه مصرف در دسترس نیست','error')}return}if(t.dataset.serviceEvents){try{const r=await api('service_event_history',{service_id:Number(t.dataset.serviceEvents)});prepareMiniLayer('previewSheet');const pv=$('previewSheet');pv.innerHTML=`<div class="preview-sheet-inner"><div class="preview-sheet-handle" data-close-preview></div><h3>🕘 رویدادهای سرویس</h3><div class="bg-admin-list">${(r.events||[]).map(x=>`<article class="my-service-info-card"><div><span>${esc(x.event_type)}</span><small>${esc(x.created_at)}</small></div><b>${esc(x.title)}</b></article>`).join('')||'<div class="u-empty">رویدادی ثبت نشده.</div>'}</div></div>`;pv.classList.add('open');pv.querySelectorAll('[data-close-preview]').forEach(x=>x.addEventListener('click',closePreviewSheet))}catch(e){showStatus(e.message||'رویدادها در دسترس نیست','error')}return}if(t.dataset.serviceSync){try{const r=await api('service_sync',{service_id:Number(t.dataset.serviceSync)});const idx=(state.services||[]).findIndex(x=>Number(x.id)===Number(t.dataset.serviceSync));if(idx>=0)state.services[idx]=r.service;renderUser();showStatus('سرویس همگام شد','success')}catch(e){showStatus(e.message||'Sync ناموفق بود','error')}return}if(t.dataset.serviceRenew){try{const r=await api('service_renew',{service_id:Number(t.dataset.serviceRenew)});state=r;currentServiceOrderId=null;currentTab='orders';currentOrderId=Number(r.order?.id||0);renderUser();showStatus('سفارش تمدید همان سرویس ساخته شد','success')}catch(e){showStatus(e.message||'تمدید انجام نشد','error')}return}if(t.dataset.serviceAddTraffic){const gb=Number(prompt('چند GB حجم اضافه؟')||0);if(!gb)return;try{const r=await api('service_add_traffic',{service_id:Number(t.dataset.serviceAddTraffic),gb});state=r;currentServiceOrderId=null;currentTab='orders';currentOrderId=Number(r.order?.id||0);renderUser();showStatus('سفارش حجم اضافه ساخته شد','success')}catch(e){showStatus(e.message||'ثبت حجم اضافه انجام نشد','error')}return}if(t.dataset.serviceAddDays){const days=Number(prompt('چند روز اضافه؟')||0);if(!days)return;try{const r=await api('service_add_days',{service_id:Number(t.dataset.serviceAddDays),days});state=r;currentServiceOrderId=null;currentTab='orders';currentOrderId=Number(r.order?.id||0);renderUser();showStatus('سفارش زمان اضافه ساخته شد','success')}catch(e){showStatus(e.message||'ثبت زمان اضافه انجام نشد','error')}return}if(t.dataset.renewOrder){try{const r=await api('renew_order',{order_id:Number(t.dataset.renewOrder)});state=r;_enhancementsHydrated=false;scheduleMiniEnhancementsHydration();currentTab='orders';currentOrderId=Number(r.order?.id||state.orders?.[0]?.id||0);renderUser();showStatus('🔄 سفارش تمدید ساخته شد','success')}catch(e){showStatus(e.message||'تمدید انجام نشد','error')}return}if(t.dataset.color){localStorage.setItem('blue_ref_color',t.dataset.color);applyTheme({...state,theme_color:t.dataset.color});showStatus('رنگ تغییر کرد')}if(t.id==='resetColor'){localStorage.removeItem('blue_ref_color');applyTheme(state);showStatus('رنگ پیش‌فرض برگشت')}if(t.id==='applyCustomColor'){const c=$('userCustomColor')?.value||'#1d9bf0';localStorage.setItem('blue_ref_color',c);applyTheme({...state,theme_color:c});showStatus('رنگ دلخواه اعمال شد')}if(t.dataset.cat){activeCategory=t.dataset.cat;document.querySelectorAll('.cat-pill').forEach(el=>el.classList.toggle('active',el.dataset.cat===activeCategory));renderShopSections()}if(t.dataset.shopSort!==undefined){shopSort=t.dataset.shopSort;document.querySelectorAll('[data-shop-sort]').forEach(el=>el.classList.toggle('active',el.dataset.shopSort===shopSort));renderShopSections()}if(t.dataset.shopToggle!==undefined){if(t.dataset.shopToggle==='instock'){shopFilterInStock=!shopFilterInStock;t.textContent=shopFilterInStock?'⚡':'📦'}else if(t.dataset.shopToggle==='featured')shopFilterFeatured=!shopFilterFeatured;else if(t.dataset.shopToggle==='wishlist'){shopFilterWishlist=!shopFilterWishlist;t.textContent=shopFilterWishlist?'❤️':'🤍'}t.classList.toggle('active');renderShopSections()}if(t.dataset.clearFilters!==undefined){searchTerm='';activeCategory='all';shopSort='newest';shopFilterInStock=false;shopFilterFeatured=false;shopFilterWishlist=false;renderShop()}if(t.id==='searchInput')return;if(t.dataset.product)showProduct(t.dataset.product);if(t.dataset.productPreview)showProduct(t.dataset.productPreview);if(t.dataset.backShop!==undefined){currentTab='shop';renderUser();miniScrollTop()}// Purchase actions are handled exclusively by the single capture-phase validated handler below (avoids duplicate listeners/API calls).
