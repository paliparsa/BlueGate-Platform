...
save_env(){
  ...
  mkdir -p "$(dirname "$ENV_FILE")"
  {
    ...
    for var in DOMAIN BOT_TOKEN BOT_USERNAME ADMIN_IDS SUPPORT_USERNAME REPO_URL APP_DIR DB_NAME DB_USER DB_PASS WEBHOOK_SECRET TELEGRAM_WEBHOOK_SECRET SWAPWALLET_CALLBACK_SECRET SERVICE_ENCRYPTION_KEY CRON_KEY THEME_COLOR BRAND_NAME FORCE_JOIN_CHANNEL ENABLE_SSL SSL_EMAIL RESEND_API_KEY RESEND_FROM_EMAIL BACKUP_KEEP DB_BACKUP_DAYS; do
      printf '%s=%q\n' "$var" "${!var:-}"
    done
  } > "$ENV_FILE"
  chmod 600 "$ENV_FILE"
...
try_extract_php_config(){
  ...
  [[ -z "$SERVICE_ENCRYPTION_KEY" ]] && SERVICE_ENCRYPTION_KEY="$(extract_string SERVICE_ENCRYPTION_KEY)"
  [[ -z "$CRON_KEY" ]] && CRON_KEY="$(extract_string CRON_KEY)"
