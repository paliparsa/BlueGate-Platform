...
install_packages(){
  ...
  apt-get update -y
  apt-get install -y nginx mariadb-server git curl unzip openssl ca-certificates rsync jq python3 \
...
acme_webroot(){ echo "${ACME_WEBROOT:-/var/www/letsencrypt}"; }
ensure_acme_webroot(){ local w; w="$(acme_webroot)"; mkdir -p "$w/.well-known/acme-challenge"; chmod 755 "$w" "$w/.well-known" "$w/.well-known/acme-challenge" 2>/dev/null || true; }
...
configure_nginx(){
  ...
  local sock conf cert_dir ssl=0 ssl_extra="" acme
  sock="$(find_php_sock)"; [[ -n "$sock" ]] || { fail "PHP-FPM socket not found"; return 1; }
  conf="/etc/nginx/sites-available/${NGINX_SITE}"; cert_dir="/etc/letsencrypt/live/${DOMAIN}"; acme="$(acme_webroot)"
  ensure_acme_webroot
...
server {
    ...
    # Let Let's Encrypt validate even when all other HTTP traffic redirects.
...
server {
    ...
    # IMPORTANT: this must stay before the generic dot-file deny rule.
    location ^~ /.well-known/acme-challenge/ {
        root ${acme};
        default_type text/plain;
        allow all;
...
EOF_NGINX
  ...
  nginx -t || { fail "nginx configuration test failed"; return 1; }
...
install_manager_command(){
  cat > "/usr/local/bin/${MANAGER_CMD}" <<EOF_MANAGER
#!/usr/bin/env bash
set -u
ENV_FILE="${ENV_FILE}"
