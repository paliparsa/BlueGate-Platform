...
configure_wizard(){
  ...
  # BlueGate v3.x compatibility: upgraded installs did not have these keys.
  # Derive them deterministically from the existing webhook secret so later
  # running configure does not invalidate provider credentials encrypted after upgrade.
  if [[ -z "$SERVICE_ENCRYPTION_KEY" ]]; then
    if [[ -f "$APP_DIR/config.php" && -n "$WEBHOOK_SECRET" ]]; then
      SERVICE_ENCRYPTION_KEY="$(printf 'bluegate-service-engine:%s' "$WEBHOOK_SECRET" | sha256sum | awk '{print $1}')"
    else
      SERVICE_ENCRYPTION_KEY="$(rand_hex 32)"
    fi
  fi
  if [[ -z "$CRON_KEY" ]]; then
    if [[ -f "$APP_DIR/config.php" && -n "$WEBHOOK_SECRET" ]]; then
      CRON_KEY="$(printf 'bluegate-cron:%s' "$WEBHOOK_SECRET" | sha256sum | awk '{print $1}' | cut -c1-48)"
    else
...
\$SERVICE_ENCRYPTION_KEY = '$(php_escape "$SERVICE_ENCRYPTION_KEY")';
\$CRON_KEY = '$(php_escape "$CRON_KEY")';
