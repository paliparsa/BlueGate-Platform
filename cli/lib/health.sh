...
health_collect(){
  ...
  age="$(file_age_sec "$APP_DIR/storage/cache/cron-provisioning.last")"; [[ "$age" -lt 300 ]] && hc_add ok "Provisioning cron" "last success ${age}s ago" || hc_add warn "Provisioning cron" "no recent success marker"
  age="$(file_age_sec "$APP_DIR/storage/cache/cron-monitoring.last")"; [[ "$age" -lt 600 ]] && hc_add ok "Monitoring cron" "last success ${age}s ago" || hc_add warn "Monitoring cron" "no recent success marker"
