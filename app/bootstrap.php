    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if ($quote !== null) {
            ...
            if ($ch === $quote) {
                ...
                if ($i + 1 < $len && $sql[$i + 1] === $quote && $quote !== '`') {
                    $buf .= $sql[++$i];
                    continue;
                }
                $quote = null;
        ...
        // MySQL/MariaDB -- comments require whitespace/control after the second dash.
...
function run_sql_file(string $path): void {
    ...
    if ($sql === false) throw new RuntimeException('SQL_FILE_READ_FAILED:'.$path);
    ...
    foreach ($statements as $index => $stmt) {
        try {
            ...
            $preview = preg_replace('/\s+/', ' ', mb_substr($stmt, 0, 320));
            throw new RuntimeException(
                'DATABASE_MIGRATION_FAILED at statement '.($index + 1).': '.($preview ?: 'SQL statement').': '.$e->getMessage(),
                0,
                $e
            );
        }
...
require_once __DIR__ . '/service_engine.php';
...
function migrate(): void {
    ...
    add_column_if_missing('orders', 'user_service_id', 'BIGINT UNSIGNED NULL AFTER is_renewal');
    add_column_if_missing('orders', 'service_action', 'VARCHAR(32) NULL AFTER user_service_id');
    add_column_if_missing('orders', 'service_action_json', 'LONGTEXT NULL AFTER service_action');
    add_column_if_missing('service_plans', 'extra_traffic_price_per_gb', 'BIGINT NOT NULL DEFAULT 0 AFTER duration_days');
    add_column_if_missing('service_plans', 'extra_day_price', 'BIGINT NOT NULL DEFAULT 0 AFTER extra_traffic_price_per_gb');
    add_column_if_missing('service_plans', 'addon_min_gb', 'INT NOT NULL DEFAULT 1 AFTER extra_day_price');
    add_column_if_missing('service_plans', 'addon_max_gb', 'INT NOT NULL DEFAULT 500 AFTER addon_min_gb');
    add_column_if_missing('service_plans', 'addon_min_days', 'INT NOT NULL DEFAULT 1 AFTER addon_max_gb');
    add_column_if_missing('service_plans', 'addon_max_days', 'INT NOT NULL DEFAULT 365 AFTER addon_min_days');
    db()->exec('CREATE TABLE IF NOT EXISTS service_usage_snapshots (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,user_service_id BIGINT UNSIGNED NOT NULL,service_instance_id BIGINT UNSIGNED NOT NULL,traffic_used_bytes BIGINT UNSIGNED NOT NULL DEFAULT 0,traffic_limit_bytes BIGINT UNSIGNED NULL,remaining_bytes BIGINT UNSIGNED NULL,expires_at DATETIME NULL,status VARCHAR(32) NOT NULL DEFAULT \'active\',raw_json LONGTEXT NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,INDEX(user_service_id,created_at),INDEX(service_instance_id,created_at),CONSTRAINT fk_usage_service FOREIGN KEY (user_service_id) REFERENCES user_services(id) ON DELETE CASCADE,CONSTRAINT fk_usage_instance FOREIGN KEY (service_instance_id) REFERENCES service_instances(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci');
...
function admin_order_keyboard(int $orderId): string {
    $o=order_by_id($orderId);$status=$o?normalize_order_status((string)$o['status']):'';$isVpn=$o&&normalize_delivery_type((string)($o['delivery_type']??''))==='vpn';$rows=[];
    if(in_array($status,['pending_payment','receipt_submitted','reviewing'],true))$rows[]=[['text'=>'👀 شروع بررسی','callback_data'=>'ord_review_'.$orderId],['text'=>'✅ تایید پرداخت','callback_data'=>'ord_paid_'.$orderId]];
    if(in_array($status,['payment_confirmed','preparing'],true)){
        if($isVpn)$rows[]=[['text'=>'⚡ ساخت و ارسال از پنل','callback_data'=>'ord_provision_'.$orderId],['text'=>'📡 وضعیت سرویس','callback_data'=>'ord_service_status_'.$orderId]];
        else $rows[]=[['text'=>'📦 آماده‌سازی','callback_data'=>'ord_prepare_'.$orderId],['text'=>'⚡️ تحویل از انبار','callback_data'=>'ord_auto_deliver_'.$orderId]];
        $rows[]=[['text'=>$isVpn?'🔗 ثبت دستی لینک ساب':'📩 تحویل دستی','callback_data'=>'ord_deliver_'.$orderId],['text'=>'📝 یادداشت داخلی','callback_data'=>'ord_note_'.$orderId]];
    } elseif($status==='delivered'&&$isVpn){$rows[]=[['text'=>'📡 وضعیت سرویس','callback_data'=>'ord_service_status_'.$orderId],['text'=>'🔗 تغییر لینک دستی','callback_data'=>'ord_deliver_'.$orderId]];}
    $rows[]=[['text'=>'🧾 تایم‌لاین','callback_data'=>'ord_timeline_'.$orderId],['text'=>'❌ رد سفارش','callback_data'=>'ord_reject_'.$orderId]];
    $rows[]=[['text'=>'📦 آرشیو','callback_data'=>'ord_archive_'.$orderId],['text'=>'🗑 حذف کامل','callback_data'=>'ord_delete_'.$orderId]];
    $rows[]=[['text'=>'🧾 سفارش‌ها','callback_data'=>'adm_orders'],['text'=>'🛒 فروشگاه ادمین','callback_data'=>'adm_shop']];return json_markup(['inline_keyboard'=>$rows]);
}
function admin_order_service_status_text(int $orderId): string {
    $o=order_by_id($orderId);if(!$o)return 'سفارش پیدا نشد.';$q=db()->prepare('SELECT * FROM user_services WHERE order_id=? ORDER BY id DESC LIMIT 1');$q->execute([$orderId]);$svc=$q->fetch();
    $txt="📡 <b>وضعیت سرویس سفارش #{$orderId}</b>\n\n";
    if(!$svc)return $txt."هنوز Client ساخته نشده است.\nاگر Mapping پلن کامل است، «ساخت و ارسال از پنل» را بزن.";
    $txt.='Service ID: <code>'.(int)$svc['id']."</code>\nوضعیت: <b>".h((string)$svc['status'])."</b>\n";
    $instances=function_exists('bluegate_service_instances')?bluegate_service_instances((int)$svc['id']):[];foreach($instances as $i){$txt.="\n• ".h((string)($i['provider_name']??('Provider #'.(int)$i['provider_id'])))."\nClient: <code>".h((string)($i['external_username']??'—'))."</code>\nTarget: <code>".h((string)($i['remote_target_id']??'—'))."</code>\nStatus: <b>".h((string)($i['status']??'—'))."</b>";if(!empty($i['subscription_url']))$txt.="\nSub: <code>".h((string)$i['subscription_url'])."</code>";$txt.="\n";}
    $jq=db()->prepare('SELECT status,attempt_count,max_attempts,last_error_message FROM provisioning_jobs WHERE user_service_id=? ORDER BY id DESC LIMIT 3');$jq->execute([(int)$svc['id']]);foreach($jq->fetchAll()?:[] as $j){$txt.="\nJob: <b>".h((string)$j['status'])."</b> ".(int)$j['attempt_count'].'/'.(int)$j['max_attempts'];if(!empty($j['last_error_message']))$txt.="\n<code>".h(mb_substr((string)$j['last_error_message'],0,300))."</code>";}return $txt;
}
...
function apply_wallet_to_order(int $orderId, int $userId): array {
    $pdo=db();$pdo->beginTransaction();try{
        ...
        add_order_event($orderId,$newFinal===0?'payment_confirmed':$status,'پرداخت از کیف پول ثبت شد','مبلغ استفاده‌شده: '.money($use),true);$pdo->commit();if($newFinal===0&&function_exists('bluegate_after_order_paid'))bluegate_after_order_paid($orderId);return order_by_id($orderId);
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
...
function apply_coupon_to_order(int $orderId, int $userId, string $code): array {
    $code=normalize_coupon_code($code);$pdo=db();$pdo->beginTransaction();try{
        ...
        add_order_event($orderId,$final===0?'payment_confirmed':'pending_payment','کد تخفیف اعمال شد',$coupon['code'].' / '.money($discount));$pdo->commit();if($final===0&&function_exists('bluegate_after_order_paid'))bluegate_after_order_paid($orderId);return order_by_id($orderId);
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
...
function update_order_status(int $orderId, string $status, string $title='', string $note='', bool $public=true): ?array {
    ...
    if($status==='payment_confirmed' && function_exists('bluegate_after_order_paid')) bluegate_after_order_paid($orderId);
    return order_by_id($orderId);
}
...
function mark_order_paid(int $orderId): ?array {
    ...
    if($q->rowCount()===1){add_order_event($orderId,'payment_confirmed','پرداخت تایید شد','',true);if(function_exists('bluegate_after_order_paid'))bluegate_after_order_paid($orderId);return order_by_id($orderId);}
...
function create_renewal_order_from_order(int $userId, int $oldOrderId): array {
    ...
    if(function_exists('bluegate_service_by_order')){try{$svc=bluegate_service_by_order($oldOrderId,$userId);$managedServiceId=$svc?(int)$svc['id']:null;}catch(Throwable $e){$managedServiceId=null;}}
    db()->prepare('UPDATE orders SET is_renewal=1, renewal_of_order_id=?, user_service_id=?, service_action=?, service_action_json=?, delivery_url=COALESCE(?,delivery_url), delivery_title=COALESCE(?,delivery_title) WHERE id=?')
        ->execute([$oldOrderId,$managedServiceId,$managedServiceId?'renew':null,$managedServiceId?json_encode(['source'=>'renew_order'],JSON_UNESCAPED_SLASHES):null,$carryUrl,$carryTitle,(int)$order['id']]);
...
function order_public_payload(array $o, bool $is_admin = false): array {
    ...
    $payload = [
        ...
        'is_renewal'=>(int)($o['is_renewal'] ?? 0), 'renewal_of_order_id'=>!empty($o['renewal_of_order_id'])?(int)$o['renewal_of_order_id']:null, 'user_service_id'=>!empty($o['user_service_id'])?(int)$o['user_service_id']:null, 'service_action'=>$o['service_action']??null,
        ...
        'service_qr_text'=>($status === 'delivered' && !empty($o['delivery_url'])) ? (string)$o['delivery_url'] : null,
