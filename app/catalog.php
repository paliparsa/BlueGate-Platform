...
function catalog_save_category(array $d): int {
    ...
    $emoji=trim((string)($d['emoji']??'🛍️'))?:'🛍️';$image=normalize_internal_media_reference(trim((string)($d['image_url']??''))?:null);$sort=(int)($d['sort_order']??99);$active=array_key_exists('is_active',$d)?(!empty($d['is_active'])?1:0):1;
...
function catalog_create_group(array $d): int {
    ...
    db()->prepare('INSERT INTO service_groups (service_id,legacy_product_id,name,slug,description,image_url,is_default,is_active,sort_order) VALUES (?,?,?,?,?,?,?,1,?)')->execute([$serviceId,$legacyProduct,$name,$slug,$d['description']??'',normalize_internal_media_reference(trim((string)($d['image_url']??''))?:null),$isDefault?1:0,(int)($d['sort_order']??99)]);return (int)db()->lastInsertId();
}
...
function catalog_create_plan(array $d): int {
    ...
    $commissionType=in_array((string)($d['commission_type']??'none'),['none','percent','fixed'],true)?(string)$d['commission_type']:'none';$commissionValue=max(0,(int)($d['commission_value']??0));$sort=(int)($d['sort_order']??99);$active=catalog_bool_value($d['is_active']??1,1);$image=normalize_internal_media_reference(trim((string)($d['image_url']??''))?:null);
...
function catalog_save_service(array $d): int {
    ...
    $desc=(string)($d['description']??$row['description']??'');$image=normalize_internal_media_reference(trim((string)($d['image_url']??$row['image_url']??''))?:null);
...
function catalog_save_group(array $d): int {
    ...
    $desc=(string)($d['description']??$row['description']??'');$image=normalize_internal_media_reference(trim((string)($d['image_url']??$row['image_url']??''))?:null);$active=catalog_bool_value($d['is_active']??$row['is_active'],(int)$row['is_active']);$sort=(int)($d['sort_order']??$row['sort_order']??99);
