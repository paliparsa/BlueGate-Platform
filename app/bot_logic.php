...
function handle_admin_callback(int $chat_id, $message_id, array $user, string $data): void {
    ...
    if ($data === 'adm_more') { send_or_edit($chat_id,$message_id,'⚙️ <b>ابزارهای بیشتر</b>\n\nابزارهای کم‌استفاده‌تر اینجا هستند.',admin_more_keyboard()); return; }
    if ($data === 'adm_ops_dash') { admin_quick_dashboard($chat_id,$message_id); return; }
    if ($data === 'adm_ops_orders') { admin_pending_orders_center($chat_id,$message_id); return; }
    if ($data === 'adm_ops_topups') { admin_topup_center($chat_id,$message_id); return; }
    if ($data === 'adm_bc') { clear_step($chat_id); admin_broadcast_home($chat_id,$message_id); return; }
    if ($data === 'adm_ntf') { clear_step($chat_id); admin_broadcast_home($chat_id,$message_id,'mini'); return; }
    if (preg_match('/^adm_bc_ch_(telegram|mini|both)$/',$data,$m)) { admin_broadcast_home($chat_id,$message_id,$m[1]); return; }
    if (preg_match('/^adm_bc_a_(telegram|mini|both)_(all|customers|vpn|recent)$/',$data,$m)) { set_step_payload($chat_id,'admin_broadcast_v2',['channel'=>$m[1],'audience'=>$m[2],'stage'=>'message']); send_or_edit($chat_id,$message_id,"✍️ <b>متن پیام را بفرست</b>\n\nکانال: <b>".h($m[1])."</b>\nمخاطب: <b>".h(admin_audience_label($m[2]))."</b>\n\nفقط خود متن را بفرست؛ هیچ فرمت خاصی لازم نیست.",json_markup(['inline_keyboard'=>[[['text'=>'❌ لغو','callback_data'=>'adm_home']]]])); return; }
    if ($data === 'adm_bc_edit') { $p=step_payload_array(get_user_by_tid($chat_id)); if(($p['channel']??'')!==''){ $p['stage']='message'; unset($p['text']);set_step_payload($chat_id,'admin_broadcast_v2',$p);send_or_edit($chat_id,$message_id,'✍️ متن جدید را بفرست.',json_markup(['inline_keyboard'=>[[['text'=>'❌ لغو','callback_data'=>'adm_home']]]]));} return; }
    if ($data === 'adm_bc_send') { $fresh=get_user_by_tid($chat_id);$p=step_payload_array($fresh?:[]);if(($fresh['step']??'')!=='admin_broadcast_v2'||empty($p['text'])){send_or_edit($chat_id,$message_id,'پیش‌نمایش منقضی شده. دوباره Broadcast را بساز.',admin_keyboard());return;}$ids=admin_audience_user_ids((string)$p['audience']);$tg=['sent'=>0,'failed'=>0];$cid=0;if(in_array($p['channel'],['telegram','both'],true))$tg=admin_send_telegram_broadcast($ids,(string)$p['text']);if(in_array($p['channel'],['mini','both'],true))$cid=admin_create_mini_campaign_for_ids($chat_id,$ids,(string)$p['text'],(string)$p['audience']);clear_step($chat_id);send_or_edit($chat_id,$message_id,"✅ <b>ارسال انجام شد</b>\n\nمخاطب: <b>".count($ids)."</b>\nTelegram موفق: <b>{$tg['sent']}</b>\nناموفق: <b>{$tg['failed']}</b>".($cid?"\nMini App Campaign: <code>#{$cid}</code>":''),admin_keyboard());return; }
    if ($data === 'adm_cust') { set_step($chat_id,'admin_customer_search_v2','');send_or_edit($chat_id,$message_id,"🔎 <b>جستجوی مشتری</b>\n\nاسم، username، ایمیل یا Telegram ID را بفرست.",json_markup(['inline_keyboard'=>[[['text'=>'❌ لغو','callback_data'=>'adm_home']]]]));return; }
    if (preg_match('/^adm_cust_id_(\d+)$/',$data,$m)) { clear_step($chat_id);admin_customer_360_bot($chat_id,$message_id,(int)$m[1]);return; }
    if (preg_match('/^adm_cadj_(\d+)_(-?\d+)$/',$data,$m)) { $uid=(int)$m[1];$amount=(int)$m[2];$u=get_user_by_id($uid);if($u){add_balance($uid,$amount,'admin_adjust','تغییر سریع اعتبار از Bot',null);send_msg((int)$u['telegram_id'],"💳 اعتبار BlueGate شما تغییر کرد.\nمقدار: <b>".money($amount)."</b>",main_menu_keyboard(is_full_admin((int)$u['telegram_id'])));}admin_customer_360_bot($chat_id,$message_id,$uid);return; }
    if (preg_match('/^adm_corders_(\d+)$/',$data,$m)) { $uid=(int)$m[1];$orders=user_orders($uid,10,true);$txt="🧾 <b>سفارش‌های مشتری</b>\n\n";$kb=[];foreach($orders as $o){$txt.='#'.$o['id'].' · '.h(order_catalog_display_name($o)).' · '.h(order_status_fa((string)$o['status']))."\n";$kb[]=[['text'=>'مدیریت #'.$o['id'],'callback_data'=>'ord_view_'.$o['id']]];}if(!$orders)$txt.='سفارشی نیست.';$kb[]=[['text'=>'🔙 مشتری','callback_data'=>'adm_cust_id_'.$uid]];send_or_edit($chat_id,$message_id,$txt,json_markup(['inline_keyboard'=>$kb]));return; }
    if (preg_match('/^adm_cmsg_(\d+)$/',$data,$m)) { set_step_payload($chat_id,'admin_customer_message_v2',['user_id'=>(int)$m[1]]);send_or_edit($chat_id,$message_id,'✉️ متن پیام مستقیم را بفرست.',json_markup(['inline_keyboard'=>[[['text'=>'❌ لغو','callback_data'=>'adm_cust_id_'.$m[1]]]]]));return; }
    if ($data === 'adm_exp') { admin_expiring_center($chat_id,$message_id,7);return; }
    if (preg_match('/^adm_exp_(1|3|7)$/',$data,$m)) { admin_expiring_center($chat_id,$message_id,(int)$m[1]);return; }
    if (preg_match('/^adm_expr_(\d+)$/',$data,$m)) { $o=order_by_id((int)$m[1]);if($o){$u=get_user_by_id((int)$o['user_id']);if($u){$name=order_catalog_display_name($o);send_msg((int)$u['telegram_id'],"⏳ <b>یادآوری تمدید</b>\n\nسرویس <b>".h($name)."</b> به‌زودی منقضی می‌شود.",json_markup(['inline_keyboard'=>[[['text'=>'🔄 تمدید سریع','callback_data'=>'svc_renew_'.(int)$o['id']],[ 'text'=>'🌐 سرویس من','callback_data'=>'svc_view_'.(int)$o['id']]]]]));}}answer_cb('','ارسال شد');admin_expiring_center($chat_id,$message_id,7);return; }
    if (preg_match('/^adm_expall_(1|3|7)$/',$data,$m)) { $days=(int)$m[1];$q=db()->prepare("SELECT o.*,u.telegram_id FROM orders o JOIN users u ON u.id=o.user_id WHERE o.status='delivered' AND o.expires_at IS NOT NULL AND o.expires_at>=NOW() AND o.expires_at<DATE_ADD(NOW(),INTERVAL ? DAY) ORDER BY o.expires_at ASC LIMIT 100");$q->execute([$days]);$sent=0;foreach($q->fetchAll()?:[] as $o){send_msg((int)$o['telegram_id'],"⏳ <b>یادآوری تمدید</b>\n\nسرویس <b>".h(order_catalog_display_name($o))."</b> به‌زودی منقضی می‌شود.",json_markup(['inline_keyboard'=>[[['text'=>'🔄 تمدید سریع','callback_data'=>'svc_renew_'.(int)$o['id']]]]]));$sent++;usleep(35000);}send_or_edit($chat_id,$message_id,"✅ یادآوری برای <b>{$sent}</b> سرویس ارسال شد.",admin_keyboard());return; }
    if (preg_match('/^adm_tu_(\d+)$/',$data,$m)) { admin_topup_view($chat_id,$message_id,(int)$m[1]);return; }
    if (preg_match('/^adm_tu_rcpt_(\d+)$/',$data,$m)) { $t=credit_topup_by_id((int)$m[1]);if($t&&!empty($t['receipt_file_id'])){try{tg('sendPhoto',['chat_id'=>$chat_id,'photo'=>(string)$t['receipt_file_id'],'caption'=>'رسید شارژ #'.(int)$t['id']]);}catch(Throwable $e){send_msg($chat_id,'نمایش رسید ممکن نشد.');}}admin_topup_view($chat_id,$message_id,(int)$m[1]);return; }
    if (preg_match('/^adm_tu_ok_(\d+)$/',$data,$m)) { try{$t=credit_topup_credit_once((int)$m[1],'تایید از Bot Admin');if($t){$u=get_user_by_id((int)$t['user_id']);if($u)send_msg((int)$u['telegram_id'],"✅ شارژ اعتبار تایید شد.\nمبلغ: <b>".money((int)$t['amount'])."</b>",main_menu_keyboard(false));}admin_topup_center($chat_id,$message_id);}catch(Throwable $e){send_or_edit($chat_id,$message_id,'تایید انجام نشد: <code>'.h($e->getMessage()).'</code>',admin_keyboard());}return; }
    if (preg_match('/^adm_tu_no_(\d+)$/',$data,$m)) { try{$t=reject_credit_topup((int)$m[1],'رد توسط ادمین از Bot');if($t){$u=get_user_by_id((int)$t['user_id']);if($u)send_msg((int)$u['telegram_id'],'❌ درخواست شارژ اعتبار رد شد.',main_menu_keyboard(false));}admin_topup_center($chat_id,$message_id);}catch(Throwable $e){send_or_edit($chat_id,$message_id,'رد درخواست انجام نشد.',admin_keyboard());}return; }
...
بعد از آماده شدن، اطلاعات تحویل برای شما ارسال می‌شود.", main_menu_keyboard(is_full_admin($o['telegram_id']))); show_admin_order($chat_id, $message_id, $oid); } return; }
    ...
    if (str_starts_with($data, 'ord_provision_')) { $oid=(int)substr($data,14); try{$r=bluegate_admin_provision_order_now($oid);$msg=($r['state']??'')==='delivered'?'✅ Client ساخته شد و لینک Subscription برای مشتری فعال شد.':'⏳ ساخت سرویس شروع شد و Retry خودکار فعال است.';send_or_edit($chat_id,$message_id,$msg,admin_order_keyboard($oid));}catch(Throwable $e){send_or_edit($chat_id,$message_id,"❌ ساخت از پنل ناموفق بود.\n<code>".h(mb_substr($e->getMessage(),0,700))."</code>",admin_order_keyboard($oid));} return; }
    if (str_starts_with($data, 'ord_service_status_')) { $oid=(int)substr($data,19); send_or_edit($chat_id,$message_id,admin_order_service_status_text($oid),admin_order_keyboard($oid)); return; }
...
function handle_step_input(int $chat_id, array $user, string $text): void {
    ...
    if ($step === 'admin_broadcast_v2') {
        $p=step_payload_array($user);if(($p['stage']??'')!=='message'){clear_step($chat_id);send_msg($chat_id,'Broadcast منقضی شده.',admin_keyboard());return;}
        $msg=trim((string)($message['text']??$message['caption']??''));if($msg===''){send_msg($chat_id,'متن پیام خالیه؛ فقط متن پیام رو بفرست.',json_markup(['inline_keyboard'=>[[['text'=>'❌ لغو','callback_data'=>'adm_home']]]]));return;}$p['text']=$msg;$p['stage']='preview';set_step_payload($chat_id,'admin_broadcast_v2',$p);$count=count(admin_audience_user_ids((string)$p['audience']));send_msg($chat_id,"👁 <b>پیش‌نمایش Broadcast</b>\n\n".$msg."\n\n────────\nکانال: <b>".h((string)$p['channel'])."</b>\nمخاطب: <b>".h(admin_audience_label((string)$p['audience']))."</b>\nتعداد تقریبی: <b>{$count}</b>",admin_broadcast_preview_keyboard());return;
    }
    if ($step === 'admin_customer_search_v2') { clear_step($chat_id);admin_customer_search_results($chat_id,$text);return; }
    if ($step === 'admin_customer_message_v2') { $p=step_payload_array($user);$uid=(int)($p['user_id']??0);$u=get_user_by_id($uid);clear_step($chat_id);if(!$u){send_msg($chat_id,'کاربر پیدا نشد.',admin_keyboard());return;}send_msg((int)$u['telegram_id'],$text,main_menu_keyboard(false));send_msg($chat_id,'✅ پیام مستقیم ارسال شد.',json_markup(['inline_keyboard'=>[[['text'=>'👤 برگشت به مشتری','callback_data'=>'adm_cust_id_'.$uid]],[['text'=>'🔙 کنترل سنتر','callback_data'=>'adm_home']]]]));return; }
